...
class TestRead(unittest.TestCase):
    def setUp(self):
        ...
        def maybe_insert(rec, aname):
            if random.random() <= 0.9:
                ...
                c = rec.pop("context_id")
                a = rec.pop("ana_id")
                attrs.clear()
                attrs[aname] = rattr.random()*100.
                for code in rec.keys():
                    self.db.attr_insert(c, code, attrs)
    ...
    def testIndexFind(self):
        ...
        query = dballe.Record()
        query.update({'ana_id': 1, 'var': "B13011", 'rep_memo': "synop"})
    ...
    def testUnsharedIndex(self):
        # Ana in one dimension, network in the other
        query = dballe.Record()
        query.update({'ana_id': 1, 'rep_memo': "synop"})
    ...
    def testConflicts(self):
        # Ana in one dimension, network in the other
        query = dballe.Record()
        query.update({'ana_id': 1, 'var': "B13011"})
        query["date"] = datetime(2007, 1, 1, 0, 0, 0)
    ...
    def testFixedIndex(self):
        # Ana in one dimension, network in the other
        query = dballe.Record()
        query.update({'ana_id': 1, 'rep_memo': "synop", 'year': 2007, 'month': 1, 'day': 1})
        ...
        vars = read(self.db.query_data(query),
                (AnaIndex(), LevelIndex(frozen=True, start=((1, dballe.MISSING_INT, dballe.MISSING_INT, dballe.MISSING_INT),) )),
                checkConflicts = False)
    ...
    def testAnaTrangeNetwork(self):
        ...
        self.assertEquals(data.dims[0][4], (5, 30., 15., None))
        self.assertEquals(data.dims[0][5], (6, 30., 25., None))
        self.assertEquals(data.dims[1][0], (0, dballe.MISSING_INT, dballe.MISSING_INT))
    ...
    def testGhostIndexes(self):
        ...
        indexes = (TimeRangeIndex(), \
                  LevelIndex(frozen=True, start=((3, 2, dballe.MISSING_INT, dballe.MISSING_INT),) ))
        query = dballe.Record()
        query['ana_id'] = 1
        query['var'] = 'B13011'
    ...
    def testBuggyExport1(self):
        indexes = (AnaIndex(),
                LevelIndex(frozen=True, start=((1, dballe.MISSING_INT, dballe.MISSING_INT), (3, 2, dballe.MISSING_INT))),
                TimeRangeIndex(),
                DateTimeIndex())
        query = dballe.Record()
