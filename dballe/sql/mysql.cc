...
struct URLParser
{
    ...
    void trace(const char* name, std::string::size_type beg,
               std::string::size_type end) const
    {
        // fprintf(stderr, "TRACE %s: %s %zd--%zd: %.*s\n", url.c_str(), name,
        // beg, end, (int)(end - beg), url.c_str() + beg);
    }
    ...
    void parse()
    {
        ...
        if (url.substr(0, 8) != "mysql://")
            error_consistency::throwf(
                "MySQL connect URL '%s' does not start with mysql://",
                url.c_str());

        size_t hostport_end = url.find('/', 8);
        if (hostport_end == string::npos)
            error_consistency::throwf("MySQL connect URL '%s' does not end the "
                                      "host:port part with a slash",
                                      url.c_str());
    ...
    void parse_qstring_keyval(std::string::size_type beg,
...
Datetime Row::as_datetime(int col) const
{
    ...
    sscanf(as_cstring(col), "%04hu-%02hhu-%02hhu %02hhu:%02hhu:%02hhu",
           &res.year, &res.month, &res.day, &res.hour, &res.minute,
           &res.second);
    return res;
}
...
Row Result::expect_one_result()
{
    if (mysql_num_rows(res) != 1)
        error_consistency::throwf("query returned %u rows instead of 1",
...
MySQLConnection::MySQLConnection()
{
    ...
    if (!db)
        throw error_mysql(nullptr, "failed to create a MYSQL object");
}
...
{
    if (forked)
        throw error_mysql("mysql handle not safe",
...
void MySQLConnection::open(const mysql::ConnectInfo& info)
{
    ...
    if (!mysql_real_connect(
            db, info.host.empty() ? nullptr : info.host.c_str(),
            info.user.c_str(), info.has_passwd ? info.passwd.c_str() : nullptr,
            info.dbname.c_str(), info.port,
            info.unix_socket.empty() ? nullptr : info.unix_socket.c_str(),
            CLIENT_REMEMBER_OPTIONS))
        throw error_mysql(db,
                          "cannot open MySQL connection to " + info.to_url());
    init_after_connect();
}
...
std::string MySQLConnection::escape(const char* str)
{
    ...
    unsigned long len = mysql_real_escape_string(
        db, const_cast<char*>(res.data()), str, str_len);
    res.resize(len);
    return res;
}
...
std::string MySQLConnection::escape(const std::string& str)
{
    ...
    unsigned long len = mysql_real_escape_string(
        db, const_cast<char*>(res.data()), str.data(), str.size());
    res.resize(len);
    return res;
}
...
std::string MySQLConnection::escape(const std::vector<uint8_t>& buf)
{
    ...
    unsigned long len = mysql_hex_string(const_cast<char*>(res.data()),
...
void MySQLConnection::exec_no_data_nothrow(const char* query) noexcept
{
    ...
    if (res != nullptr)
    {
        fprintf(stderr, "query '%s' returned %u rows instead of zero", query,
                (unsigned)mysql_num_rows(res));
    ...
    if (mysql_errno(db))
    {
        fprintf(stderr, "cannot store result of query '%s': %s", query,
                mysql_error(db));
        mysql_free_result(res);
        return;
    }
...
void MySQLConnection::exec_no_data(const std::string& query)
{
    ...
    if (res)
        ...
        error_mysql::throwf(db, "cannot store result of query '%s'",
                            query.c_str());
}
...
void MySQLConnection::exec_use(const char* query,
...
void MySQLConnection::exec_use(const std::string& query,
                               std::function<void(const mysql::Row&)> dest)
{
    using namespace dballe::sql::mysql;
    trace_query("exec_use: %s\n", query.c_str());
    ...
    if (!res)
    {
        if (mysql_errno(db))
            ...
            error_consistency::throwf("query '%s' returned no data",
                                      query.c_str());
    }
    send_result(move(res), dest);
}

void MySQLConnection::send_result(mysql::Result&& res,
                                  std::function<void(const mysql::Row&)> dest)
{
    using namespace dballe::sql::mysql;
    check_connection();
    ...
    while (Row row = res.fetch())
    {
        ...
        catch (...)
        {
            ...
            // If there is a network connectivity problem, we will never know
            // it.
            //
            // See:
...
struct MySQLTransaction : public Transaction
{
    ...
    void lock_table(const char* name) override
    {
        ...
        //   active transaction before attempting to lock the tables.
        //
...
std::string MySQLConnection::get_setting(const std::string& key)
{
    ...
    if (res.rowcount() > 1)
        error_consistency::throwf("got %d results instead of 1 executing %s",
                                  res.rowcount(), query.c_str());
    Row row = res.fetch();
    return row.as_string(0);
}

void MySQLConnection::set_setting(const std::string& key,
                                  const std::string& value)
{
    if (!has_table("dballe_settings"))
        exec_no_data("CREATE TABLE dballe_settings (`key` CHAR(64) NOT NULL "
                     "PRIMARY KEY, value CHAR(64) NOT NULL)");
...
void MySQLConnection::execute(const std::string& query) { exec_no_data(query); }
...
void MySQLConnection::explain(const std::string& query, FILE* out)
{
    ...
    fprintf(out, "%s\n", explain_query.c_str());
    fprintf(out,
            "sid\tstype\ttable\ttype\tpos_ks\tkey\tkeylen\tref\trows\textra\n");
    exec_use(explain_query, [&](const Row& row) {
        fprintf(out, "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n",
                row.as_cstring(0), row.as_cstring(1), row.as_cstring(2),
                row.as_cstring(3), row.as_cstring(4), row.as_cstring(5),
                row.as_cstring(6), row.as_cstring(7), row.as_cstring(8),
                row.as_cstring(9));
    ...
    exec_use("SHOW WARNINGS", [&](const Row& row) {
        fprintf(out, "%s\t%s\t%s\n", row.as_cstring(0), row.as_cstring(1),
                row.as_cstring(2));
    });
}
