...
struct SQLiteFixture : Fixture
{
    ...
    SQLiteFixture() : conn(SQLiteConnection::create()) { conn->open_memory(); }
...
void Tests::register_tests()
{
    ...
    add_method("bytes", [](Fixture& f) {
        ...
        s->execute_one([&]() { val = s->column_blob(0); });
    ...
    add_method("auto_increment", [](Fixture& f) {
        // Test auto_increment
        f.conn->exec(
            "CREATE TABLE dballe_testai (id INTEGER PRIMARY KEY, val INTEGER)");
    ...
    add_method("connect", [](Fixture& f) {
        auto conn =
            Connection::create(*DBConnectOptions::create("sqlite:test.sqlite"));
        wassert_true(conn->server_type == sql::ServerType::SQLITE);
        conn = Connection::create(
            *DBConnectOptions::create("sqlite://test.sqlite"));
        wassert_true(conn->server_type == sql::ServerType::SQLITE);
        conn = Connection::create(
            *DBConnectOptions::create("sqlite://test.sqlite?a=b,c=d"));
        wassert_true(conn->server_type == sql::ServerType::SQLITE);
        conn = Connection::create(
            *DBConnectOptions::create("sqlite:test.sqlite?a=b,c=d"));
        wassert_true(conn->server_type == sql::ServerType::SQLITE);
