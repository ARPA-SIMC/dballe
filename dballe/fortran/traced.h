...
struct Tracer
{
    ...
    virtual std::unique_ptr<API> wrap_api(int handle,
                                          std::unique_ptr<API> api) = 0;

    std::unique_ptr<API> begin(int dbahandle, int handle,
                               const DBConnectOptions& options,
                               const char* anaflag, const char* dataflag,
                               const char* attrflag);
    std::unique_ptr<API> begin_messages(int handle, const char* filename,
    ...
    virtual void log_begin(int dbahandle, int handle, const char* anaflag,
                           const char* dataflag, const char* attrflag)  = 0;
    virtual void log_begin_messages(int handle, const char* filename,
                                    const char* mode, const char* type) = 0;
...
struct TracedAPI : public API
{
    ...
    TracedAPI(FileTracer& tracer, const std::string& name,
              std::unique_ptr<API> api);
