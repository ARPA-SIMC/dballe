...
struct InputFile
{
    ...
    const Message& msg() const { return *current_msg[current_msg_idx]; }
};
...
struct QuantesonoOperation
    : public CursorOperation<dballe::db::v7::cursor::Stations>
{
    const DbAPI& api;

    QuantesonoOperation(const DbAPI& api) : api(api) {}

    int run()
    {
        cursor = dballe::db::v7::cursor::Stations::downcast(
            api.tr->query_stations(api.input_query));
...
template <> struct CursorTraits<db::v7::cursor::StationData>
{
    static inline std::shared_ptr<db::v7::cursor::StationData>
    query(db::Transaction& tr, const core::Query& query)
    {
        return db::v7::cursor::StationData::downcast(
            tr.query_station_data(query));
    }
    static inline void attr_insert(db::Transaction& tr, int id,
                                   const Values& values)
    {
        tr.attr_insert_station(id, values);
    }
    static inline void attr_remove(db::Transaction& tr, int id,
                                   const std::vector<wreport::Varcode>& attrs)
    {
        tr.attr_remove_station(id, attrs);
    }
};

template <> struct CursorTraits<db::v7::cursor::Data>
{
    static inline std::shared_ptr<db::v7::cursor::Data>
    query(db::Transaction& tr, const core::Query& query)
    {
...
struct VoglioquestoOperation : public CursorOperation<Cursor>
{
    ...
    VoglioquestoOperation(const DbAPI& api) : api(api) {}
    ...
    {
        if (next_data_ended)
            throw error_consistency("insert_attributes called after next_data "
                                    "returned end of data");
        CursorTraits<Cursor>::attr_insert(
            *api.tr, this->cursor->attr_reference_id(), qcinput);
        valid_cached_attrs = false;
    }
    void remove_attributes() override
    {
        if (next_data_ended)
            throw error_consistency("remove_attributes called after next_data "
                                    "returned end of data");
        CursorTraits<Cursor>::attr_remove(*api.tr,
                                          this->cursor->attr_reference_id(),
                                          api.selected_attr_codes);
        valid_cached_attrs = false;
    }
};
...
struct VarID
{
    ...
    bool station;
    size_t id;
    VarID(wreport::Varcode code, bool station, size_t id)
        : code(code), station(station), id(id)
    {
    }
};
...
struct PrendiloOperation : public Operation
{
    ...
    PrendiloOperation(DbAPI& api) : api(api)
    {
    ...
    void run()
    {
        // db::Transaction& tr, dballe::core::Data& input, bool station_context,
        // unsigned perms)
        ...
        if (api.station_context)
        {
            ...
            for (const auto& v : api.input_data.values)
                last_inserted_varids.push_back(
                    VarID(v.code(), true, v.data_id));
        }
        else
        {
    ...
    void insert_attributes(Values& qcinput) override
    {
        ...
        else
        {
            if (varcode == 0)
                throw error_consistency(
                    "please set *var_related before calling insert_attributes "
                    "after setting multiple variables in a single insert_data");
            ...
            if (data_id == MISSING_INT)
                error_consistency::throwf(
                    "cannot insert attributes for *var_related=%01d%02d%03d: "
                    "the last insert_data inserted %zd variables, none of "
                    "which match *var_related",
                    WR_VAR_FXY(varcode), last_inserted_varids.size());
        }
        if (is_station)
    ...
    {
        throw error_consistency(
            "remove_attributes cannot be called after a insert_data");
    }
...
struct VaridOperation : public Operation
{
    ...
    void query_attributes(Attributes& dest) override
    {
        if (!varid)
            throw error_consistency(
                "query_attributes called with an invalid *context_id");
    ...
    int enqi(const char* param) const override
    {
        throw wreport::error_unimplemented(
            "Varid::enqi handle *params or forward to previous operation");
    }
    double enqd(const char* param) const override
    {
        throw wreport::error_unimplemented(
            "Varid::enqd handle *params or forward to previous operation");
    }
    bool enqc(const char* param, char* res, unsigned res_len) const override
    {
        throw wreport::error_unimplemented(
            "Varid::enqc handle *params or forward to previous operation");
    }
    void enqlevel(int& ltype1, int& l1, int& ltype2, int& l2) const override
    {
        throw wreport::error_unimplemented(
            "VaridOperation::enqlevel forward to previous operation");
    }
    void enqtimerange(int& ptype, int& p1, int& p2) const override
    {
        throw wreport::error_unimplemented(
            "VaridOperation::enqtimerange forward to previous operation");
    }
    void enqdate(int& year, int& month, int& day, int& hour, int& min,
                 int& sec) const override
    {
        throw wreport::error_unimplemented(
            "VaridOperation::enqdata forward to previous operation");
    }
};

} // namespace

DbAPI::DbAPI(std::shared_ptr<db::Transaction> tr, const char* anaflag,
             const char* dataflag, const char* attrflag)
    : DbAPI(tr, compute_permissions(anaflag, dataflag, attrflag))
{
}

DbAPI::DbAPI(std::shared_ptr<db::Transaction> tr, unsigned perms) : tr(tr)
{
    this->perms = perms;
}

DbAPI::~DbAPI() { shutdown(false); }
...
void DbAPI::commit() { shutdown(true); }
...
int DbAPI::query_data()
{
    ...
    if (station_context)
        return reset_operation(
            new VoglioquestoOperation<db::v7::cursor::StationData>(*this));
    else
        return reset_operation(
            new VoglioquestoOperation<db::v7::cursor::Data>(*this));
}

void DbAPI::insert_data()
{
    if (perms & PERM_DATA_RO)
        throw error_consistency("idba_insert_data cannot be called with the "
                                "database open in data readonly mode");
...
{
    if (!(perms & PERM_DATA_WRITE))
        throw error_consistency("remove_data must be called with the database "
                                "open in data write mode");
...
void DbAPI::messages_open_input(const char* filename, const char* mode,
                                Encoding format, bool simplified)
{
    ...
    if ((perms & PERM_ANA_RO) || (perms & PERM_DATA_RO))
        throw error_consistency("messages_open must be called on a session "
                                "with writable station and data");
...
void DbAPI::messages_open_output(const char* filename, const char* mode,
                                 Encoding format)
{
    if (strchr(mode, 'w') == NULL && strchr(mode, 'a') == NULL)
        throw error_consistency(
            "output files should be open with 'w' or 'a' mode");
...
bool DbAPI::messages_read_next()
{
    if (!input_file)
        throw error_consistency(
            "messages_read_next called but there are no open input files");
    if (!input_file->next())
        return false;
...
std::unique_ptr<API> DbAPI::fortran_connect(const DBConnectOptions& options,
                                            const char* anaflag,
                                            const char* dataflag,
                                            const char* attrflag)
{
    ...
    bool readonly =
        !(perms &
          (fortran::DbAPI::PERM_ANA_WRITE | fortran::DbAPI::PERM_DATA_ADD |
           fortran::DbAPI::PERM_DATA_WRITE | fortran::DbAPI::PERM_ATTR_WRITE));
