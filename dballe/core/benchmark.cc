...
static std::string format_clockdiff(const struct timespec& begin,
...
void Throughput::run(Progress& progress, Task& task)
{
    ...
    try
    {
        ...
        time_at_end.tv_nsec =
            time_at_start.tv_nsec +
            ((long)floor(run_time * 1000000000.0) % 1000000000);
        time_at_end.tv_sec = time_at_start.tv_sec +
                             time_at_end.tv_nsec / 1000000000 +
                             (long)floor(run_time);
        ...
        for (; true; ++times_run)
        {
            ...
            if (time_cur.tv_sec > time_at_end.tv_sec)
                break;
            if (time_cur.tv_sec == time_at_end.tv_sec &&
                time_cur.tv_nsec > time_at_end.tv_nsec)
                break;
            task.run_once();
        }

        run_time = time_cur.tv_sec - time_at_start.tv_sec +
                   (time_cur.tv_nsec - time_at_start.tv_nsec) / 1000000000.0;
    }
    catch (std::exception& e)
    {
        progress.test_failed(task, e);
...
Benchmark::Benchmark() : progress(make_shared<BasicProgress>()) {}
...
void Benchmark::print_timings()
{
    for (auto& t : timeit_tasks)
    {
        ...
        fprintf(stdout, "%s,%u,%s\n", t.task_name.c_str(), t.repetitions,
                time.c_str());
    }
    for (auto& t : throughput_tasks)
    {
        fprintf(stdout, "%s,%.2f,%d\n", t.task_name.c_str(), t.run_time,
                t.times_run);
    }
    /*
    for (auto& t: tasks)
    {
        fprintf(stdout, "%s.%s: %d runs, user: %.2fs (%.1f%%), sys: %.2fs
    (%.1f%%), total: %.2fs (%.1f%%)\n", name.c_str(), t->name.c_str(),
                ...
                (t->utime + t->stime) * 100.0 / (task_main.utime +
    task_main.stime));
    }
    */
}
...
{
    fprintf(out, "%u times in %.2fs: %.2f/s.\n", t.times_run, t.run_time,
            (double)t.times_run / t.run_time);
}
...
void Messages::load(const std::string& pathname, dballe::Encoding encoding,
