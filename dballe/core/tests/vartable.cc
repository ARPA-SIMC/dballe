...
void to::test<1>()
{
	...
	tests::LocalEnv le("DBA_TABLES", testdatadir ? testdatadir : ".");

	const Vartable* table = Vartable::get("test-crex-table");

	try {
		table->query(DBA_VAR(0, 2, 99));
	} catch (error_notfound& e) {
		ensure_contains(e.what(), "002099");
	}

	Varinfo info = table->query(DBA_VAR(0, 1, 6));
	ensure_equals(info->var, DBA_VAR(0, 1, 6));
	...
	info = table->query(DBA_VAR(0, 2, 114));
	ensure_equals(info->var, DBA_VAR(0, 2, 114));
	...
	info = table->query(DBA_VAR(0, 2, 153));
	ensure_equals(info->var, DBA_VAR(0, 2, 153));
...
void to::test<2>()
{
	...
	info = table->query(DBA_VAR(0, 11, 35));
	ensure_equals(info->var, DBA_VAR(0, 11, 35));
	...
	info = table->query(DBA_VAR(0, 7, 31));
	ensure_equals(info->var, DBA_VAR(0, 7, 31));
