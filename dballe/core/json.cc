...
void JSONWriter::reset() { stack.clear(); }
...
{
    start_list();
    if (val.ltype1 != MISSING_INT)
        add(val.ltype1);
    else
        add_null();
    if (val.l1 != MISSING_INT)
        add(val.l1);
    else
        add_null();
    if (val.ltype2 != MISSING_INT)
        add(val.ltype2);
    else
        add_null();
    if (val.l2 != MISSING_INT)
        add(val.l2);
    else
        add_null();
    end_list();
...
{
    start_list();
    if (val.pind != MISSING_INT)
        add(val.pind);
    else
        add_null();
    if (val.p1 != MISSING_INT)
        add(val.p1);
    else
        add_null();
    if (val.p2 != MISSING_INT)
        add(val.p2);
    else
        add_null();
    end_list();
...
{
    start_list();
    if (val.lat != MISSING_INT)
        add(val.lat);
    else
        add_null();
    if (val.lon != MISSING_INT)
        add(val.lon);
    else
        add_null();
    end_list();
...
void JSONWriter::add_values(const Values& values)
{
    ...
    for (const auto& var : values)
    {
        ...
        if (var->next_attr())
        {
            add("a");
            start_mapping();
            for (const wreport::Var* attr = var->next_attr(); attr;
                 attr                     = attr->next_attr())
            {
...
void Stream::expect_token(const char* token)
{
    for (const char* s = token; *s; ++s)
    {
        ...
        {
            if (c == EOF)
                // throw JSONParseException(str::fmtf("end of file reached
                // looking for %s in %s", s, expected));
                throw JSONParseException("unexpected end of file reached");
            else
                // throw JSONParseException(str::fmtf("unexpected character '%c'
                // looking for %s in %s", c, s, expected));
            ...
            parse_array([&] {
                switch (identify_next())
                {
                    case JSON_NUMBER:
                        ...
                        {
                            case 0:
                                res.year = parse_unsigned<unsigned short>();
                                break;
                            case 1:
                                res.month = parse_signed<unsigned char>();
                                break;
                            case 2:
                                res.day = parse_signed<unsigned char>();
                                break;
                            case 3:
                                res.hour = parse_signed<unsigned char>();
                                break;
                            case 4:
                                res.minute = parse_signed<unsigned char>();
                                break;
                            case 5:
                                res.second = parse_signed<unsigned char>();
                                break;
                            default:
                                throw JSONParseException(
                                    "extra element in Datetime array");
                        }
                        break;
                    case JSON_NULL: expect_token("null"); break;
                    default:
                        throw JSONParseException(
                            "unexpected element in Datetime array");
                }
                ++idx;
            });
            return res;
...
DatetimeRange Stream::parse_datetimerange()
{
    ...
    parse_array([&] {
        switch (idx)
        {
            ...
            default:
                throw JSONParseException(
                    "extra element in DatetimeRange array");
        }
        ++idx;
    });
    return res;
...
static void parse_value(json::Stream& in, JSONReader& e)
{
    switch (in.identify_next())
    {
        ...
        case json::JSON_ARRAY:
            e.on_start_list();
            in.parse_array([&] { parse_value(in, e); });
            e.on_end_list();
            break;
        case json::JSON_STRING: e.on_add_string(in.parse_string()); break;
