...
Varmatch::Varmatch(wreport::Varcode code) : code(code) {}
...
template <typename T, typename OP> struct Op : public Varmatch
{
    OP op;
    T val;
...
template <typename T>
static std::unique_ptr<Varmatch> make_op(Varcode code, const std::string& op,
                                         const T& val)
{
    if (op == "<")
        ...
        return std::unique_ptr<Varmatch>(
            new Op<T, greater_equal<T>>(code, val));
    else if (op == "=" || op == "==")
        ...
        error_consistency::throwf("cannot understand comparison operator '%s'",
                                  op.c_str());
}
...
template <typename T>
static std::unique_ptr<Varmatch>
make_between(Varcode code, const std::string& op1, const std::string& op2,
             const T& min, const T& max)
{
    if (op1 == "<" && op2 == "<")
        return std::unique_ptr<Varmatch>(
            new Between<T, less<T>, less<T>>(code, min, max));
    else if (op1 == "<=" && op2 == "<=")
        return std::unique_ptr<Varmatch>(
            new Between<T, less_equal<T>, less_equal<T>>(code, min, max));
    else if (op1 == "<" && op2 == "<=")
        return std::unique_ptr<Varmatch>(
            new Between<T, less<T>, less_equal<T>>(code, min, max));
    else if (op1 == "<=" && op2 == "<")
        return std::unique_ptr<Varmatch>(
            new Between<T, less_equal<T>, less<T>>(code, min, max));
    else
        error_consistency::throwf(
            "cannot understand comparison operators '%s' and '%s'", op1.c_str(),
            op2.c_str());
}
...
std::unique_ptr<Varmatch> Varmatch::parse(const std::string& filter)
{
    ...
    if (sep1_begin == string::npos)
        error_consistency::throwf("cannot find any operator in filter '%s'",
                                  filter.c_str());

    size_t sep1_end = filter.find_first_not_of("<=>!", sep1_begin);
    if (sep1_end == string::npos)
        error_consistency::throwf(
            "cannot find end of first operator in filter '%s'", filter.c_str());
    ...
    if (sep2_begin != string::npos)
    {
        ...
        if (sep2_end == string::npos)
            error_consistency::throwf(
                "cannot find end of second operator in filter '%s'",
                filter.c_str());
        Varcode code = resolve_varcode(
            filter.substr(sep1_end, sep2_begin - sep1_end).c_str());
        ...
        switch (info->type)
        {
            ...
            case Vartype::Binary:
                error_consistency::throwf(
                    "cannot apply filter '%s' to a binary variable",
                    filter.c_str());
    ...
    else
    {
        ...
        switch (info->type)
        {
            ...
            case Vartype::Integer:
            case Vartype::Decimal: {
                int val = info->encode_decimal(
                    strtod(filter.substr(sep1_end).c_str(), NULL));
