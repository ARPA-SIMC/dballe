...
dba_err aof_read_flight(const uint32_t* obs, int obs_len, dba_msg* out)
{
	...
	if (OBS(24) != AOF_UNDEF)
	{
		...
		DBA_RUN_OR_RETURN(dba_convert_icao_to_press(OBS(24), &press));
	}
	else if (OBS(20) != AOF_UNDEF)
	{
		double height;
		press = OBS(20);
		DBA_RUN_OR_RETURN(dba_convert_press_to_icao(press, &height));
		DBA_RUN_OR_RETURN(dba_msg_set_height(msg, height, -1));
	}
