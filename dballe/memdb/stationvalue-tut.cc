...
template<> template<> void to::test<1>()
{
    ...
    const StationValue& sv = svalues.insert_or_replace(stf, newvar(WR_VAR(0, 1, 102), 28.5));
    wassert(actual(&sv.station) == &stf);
    wassert(actual(sv.var->code()) == WR_VAR(0, 1, 102));
    wassert(actual(sv.var->enqd()) == 28.5);

    // Replacing a value should reuse an existing one
    const StationValue& sv1 = svalues.insert_or_replace(stf, newvar(WR_VAR(0, 1, 102), 29.5));
    wassert(actual(&sv1) == &sv);
    wassert(actual(&sv1.station) == &stf);
    wassert(actual(sv1.var->code()) == WR_VAR(0, 1, 102));
    wassert(actual(sv1.var->enqd()) == 28.5);
}
