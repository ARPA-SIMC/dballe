...
public:
    ...
    size_t insert_or_replace(const Station& station, const LevTr& levtr, const Datetime& datetime, std::auto_ptr<wreport::Var> var);

    /// Insert a new value, or replace an existing one
    size_t insert_or_replace(const Station& station, const LevTr& levtr, const Datetime& datetime, const wreport::Var& var);
