...
void ActualCursor::data_context_matches(const Data& expected)
{
    ...
    if (!c)
        throw TestFailed("cursor is not an instance of CursorData");
...
void ActualDB<DB>::try_data_query(const Query& query, unsigned expected)
{
    ...
    unsigned count =
        dynamic_cast<db::CursorData*>(cur.get())->test_iterate(/* stderr */);
    wassert(actual(count) == expected);
}

template <typename DB>
void ActualDB<DB>::try_station_query(const std::string& query,
                                     unsigned expected)
{
    // Run the query
    ...
    wassert(actual(cur->remaining()) == expected);
    unsigned count =
        dynamic_cast<db::CursorStation*>(cur.get())->test_iterate(/* stderr */);
    wassert(actual(count) == expected);
}

template <typename DB>
void ActualDB<DB>::try_summary_query(const std::string& query,
                                     unsigned expected,
...
template <typename DB> BaseDBFixture<DB>::~BaseDBFixture()
{
    if (db && getenv("PAUSE") == nullptr)
        db->disappear();
}

template <typename DB> bool BaseDBFixture<DB>::has_driver()
{
    return tests::has_driver(backend);
}

template <typename DB> void BaseDBFixture<DB>::create_db()
{
...
template <typename DB> void BaseDBFixture<DB>::test_setup()
{
    Fixture::test_setup();
    if (!has_driver())
        throw TestSkipped();
...
template <typename DB> void BaseDBFixture<DB>::test_teardown()
{
    if (destroys_db)
    {
        db->disappear();
...
template <typename DB> void EmptyTransactionFixture<DB>::test_setup()
{
    BaseDBFixture<DB>::test_setup();
    tr = dynamic_pointer_cast<typename DB::TR>(this->db->test_transaction());
}

template <typename DB> void EmptyTransactionFixture<DB>::test_teardown()
{
    if (tr)
        tr->rollback();
    tr.reset();
...
template <typename DB> void DBFixture<DB>::test_setup()
{
    BaseDBFixture<DB>::test_setup();
                                                    ...
                                                    bool wipe)
{
    ...
    options->wipe = wipe;
    return std::dynamic_pointer_cast<dballe::db::v7::DB>(
        dballe::DB::connect(*options));
}
...
{
    return ActualDB<dballe::db::Transaction>(
        std::static_pointer_cast<dballe::db::Transaction>(actual));
}
