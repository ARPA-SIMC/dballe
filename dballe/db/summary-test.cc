...
Tests<V7DB, SummaryMemory> tg1("db_summary_memory_v7_sqlite_summary_memory",
                               "SQLITE");
Tests<V7DB, DBSummaryMemory> tg2("db_summary_memory_v7_sqlite_dbsummary_memory",
                                 "SQLITE");
#ifdef HAVE_LIBPQ
Tests<V7DB, SummaryMemory> tg3("db_summary_memory_v7_postgresql_summary_memory",
                               "POSTGRESQL");
Tests<V7DB, DBSummaryMemory>
    tg4("db_summary_memory_v7_postgresql_dbsummary_memory", "POSTGRESQL");
#endif
#ifdef HAVE_MYSQL
Tests<V7DB, SummaryMemory> tg5("db_summary_memory_v7_mysql_summary_memory",
                               "MYSQL");
Tests<V7DB, DBSummaryMemory> tg6("db_summary_memory_v7_mysql_dbsummary_memory",
                                 "MYSQL");
#endif

std::unique_ptr<Summary> other_summary(const DBSummaryMemory&)
{
    return std::unique_ptr<Summary>(new SummaryMemory);
}
std::unique_ptr<DBSummary> other_summary(const SummaryMemory&)
{
    return std::unique_ptr<DBSummary>(new DBSummaryMemory);
}

#ifdef HAVE_XAPIAN
Tests<V7DB, SummaryXapian> tg7("db_summary_xapian_v7_sqlite_summary_disk",
                               "SQLITE");
Tests<V7DB, DBSummaryXapian> tg8("db_summary_xapian_v7_sqlite_dbsummary_disk",
                                 "SQLITE");
#ifdef HAVE_LIBPQ
Tests<V7DB, SummaryXapian> tg9("db_summary_xapian_v7_postgresql_summary_disk",
                               "POSTGRESQL");
Tests<V7DB, DBSummaryXapian>
    tg10("db_summary_xapian_v7_postgresql_dbsummary_disk", "POSTGRESQL");
#endif
#ifdef HAVE_MYSQL
Tests<V7DB, SummaryXapian> tg11("db_summary_xapian_v7_mysql_summary_disk",
                                "MYSQL");
Tests<V7DB, DBSummaryXapian> tg12("db_summary_xapian_v7_mysql_dbsummary_disk",
                                  "MYSQL");
#endif

std::unique_ptr<Summary> other_summary(const DBSummaryXapian&)
{
    return std::unique_ptr<Summary>(new SummaryXapian);
}
std::unique_ptr<DBSummary> other_summary(const SummaryXapian&)
{
    return std::unique_ptr<DBSummary>(new DBSummaryXapian);
}
#endif

void station_id_isset(const Station& station) {}
void station_id_isset(const DBStation& station)
{
...
void Tests<DB, BACKEND>::register_tests()
{
    ...
    this->add_method("summary_msg", [](Fixture& f) {
        ...
        // Summarise a message
        impl::Messages msgs = dballe::tests::read_msgs(
            "bufr/synop-rad1.bufr", Encoding::BUFR, "accurate");
        s.add_messages(msgs);
    ...
    this->add_method("json_summary", [](Fixture& f) {
        ...
        wassert(
            actual(json.str()) ==
            R"({"e":[{"s":{"r":"test","c":[4450000,1150000]},"v":[{"l":[1,null,null,null],"t":[254,0,0],"v":368,"d":[[2018,1,1,0,0,0],[2018,7,1,0,0,0]],"c":12},{"l":[1,null,null,null],"t":[254,0,0],"v":369,"d":[[2018,1,1,0,0,0],[2018,7,1,0,0,0]],"c":12}]}]})");
        ...
        wassert(actual(get_stations(summary1).size()) ==
                get_stations(summary).size());
        wassert(actual(get_reports(summary1)) == get_reports(summary));
        wassert(actual(get_levels(summary1)) == get_levels(summary));
        wassert(actual(get_tranges(summary1)) == get_tranges(summary));
        wassert(actual(get_varcodes(summary1)) == get_varcodes(summary));
    ...
    this->add_method("datetime_intersect", [](Fixture& f) {
        ...
        s.add(station, vd,
              DatetimeRange(Datetime(2020, 1, 1), Datetime(2020, 2, 1)), 10);
