...
struct Fixture : EmptyTransactionFixture<V7DB>
{
    ...
    Fixture(const char* backend) : EmptyTransactionFixture(backend) {}
    ...
    void create_db() override
    {
        ...
        auto t = dynamic_pointer_cast<dballe::db::v7::Transaction>(
            db->transaction());
...
void Tests::register_tests()
{
    ...
    add_method("insert", [](Fixture& f) {
        ...
        // Insert a datum
        {
            ...
            wassert(da.insert(trc, f.sde1.id, Datetime(2001, 2, 3, 4, 5, 6),
                              vars, false));
        ...
        // Insert another datum
        {
            ...
            wassert(da.insert(trc, f.sde2.id, Datetime(2002, 3, 4, 5, 6, 7),
                              vars, false));
        ...
        //    std::vector<batch::MeasuredDatum> vars;
        //    vars.emplace_back(f.lt1, &var);
        //    auto e = wassert_throws(std::runtime_error, da.insert(*f.tr,
        //    f.tr->station().get_id(*f.tr, f.sde1), Datetime(2001, 2, 3, 4, 5,
        //    6), vars)); wassert(actual(e.what()).matches("refusing to
    ...
    add_method("attrs", [](Fixture& f) {
        ...
        std::vector<batch::MeasuredDatum> vars;
        vars.emplace_back(f.lt1, &var);
        wassert(da.insert(trc, f.sde1.id, Datetime(2001, 2, 3, 4, 5, 6), vars,
                          true));
        int id = vars[0].id;
