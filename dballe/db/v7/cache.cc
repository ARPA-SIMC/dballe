...
const LevTrEntry* LevTrCache::insert(std::unique_ptr<LevTrEntry> e)
{
    if (e->id == MISSING_INT)
        throw std::runtime_error(
            "station to cache in transaction state must have a database ID");

    auto i = this->by_id.find(e->id);
    if (i != this->by_id.end())
    {
        // StationCache do not move: if we have a match on the ID, we just need
        // to enforce that there is no mismatch on the station data
        if (*i->second != *e)
            throw std::runtime_error("cannot replace a cached DB entry with "
                                     "one with the same ID and different data");
        return i->second;
    }
