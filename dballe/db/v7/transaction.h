...
protected:
    ...
    void add_msg_to_batch(Tracer<>& trc, const Message& message,
...
public:
    ...
    Transaction(std::shared_ptr<v7::DB> db,
    ...
    query_stations(const Query& query) override;
    std::shared_ptr<dballe::CursorStationData>
    query_station_data(const Query& query) override;
    std::shared_ptr<dballe::CursorData> query_data(const Query& query) override;
    std::shared_ptr<dballe::CursorSummary>
    query_summary(const Query& query) override;
    std::shared_ptr<dballe::CursorMessage>
    query_messages(const Query& query) override;
    void attr_query_station(
        int data_id,
        std::function<void(std::unique_ptr<wreport::Var>)> dest) override;
    void attr_query_data(
        int data_id,
        std::function<void(std::unique_ptr<wreport::Var>)> dest) override;

    void insert_station_data(dballe::Data& vals,
                             const dballe::DBInsertOptions& opts =
                                 dballe::DBInsertOptions::defaults) override;
    void insert_data(dballe::Data& vals,
                     const dballe::DBInsertOptions& opts =
                         dballe::DBInsertOptions::defaults) override;
    ...
    void import_message(const Message& message,
                        const dballe::DBImportOptions& opts) override;
    void import_messages(const std::vector<std::shared_ptr<Message>>& msgs,
                         const dballe::DBImportOptions& opts) override;
    void update_repinfo(const char* repinfo_file, int* added, int* deleted,
