...
void Tests::register_tests()
{
    ...
    add_method("empty", [](Fixture& f) {
        ...
        batch::Station* station = wcallchecked(
            batch.get_station(trc, "synop", Coords(11.0, 45.0), Ident()));
        ...
        station = wcallchecked(
            batch.get_station(trc, "synop", Coords(11.0, 45.0), "AB123"));
        wassert(actual(station->report) == "synop");
        wassert(actual(station->id) == MISSING_INT);
        wassert(actual(station->coords) == Coords(11.0, 45.0));
    ...
    add_method("reuse", [](Fixture& f) {
        ...
        batch::Station* station1 = wcallchecked(
            batch.get_station(trc, "synop", Coords(11.0, 45.0), Ident()));
    ...
    add_method("from_db", [](Fixture& f) {
        ...
        batch::Station* station =
            wcallchecked(batch.get_station(trc, "synop", coords, Ident()));
        wassert(actual(station->report) == "synop");
        wassert(actual(station->id) != MISSING_INT);
        ...
        auto measured_data =
            station->get_measured_data(trc, Datetime(2013, 10, 16, 10));
    ...
    add_method("import", [](Fixture& f) {
        db::v7::Tracer<> trc;
        impl::Messages msgs1 =
            read_msgs("bufr/test-airep1.bufr", Encoding::BUFR);
    ...
    add_method("insert", [](Fixture& f) {
        ...
        int id_levtr =
            f.tr->levtr().obtain_id(trc, LevTrEntry(Level(1), Trange(254)));
