...
void PostgreSQLRepinfo::read_cache()
{
    ...
    auto stm = conn.exec("SELECT id, memo, description, prio, descriptor, "
                         "tablea FROM repinfo ORDER BY id");
    for (unsigned row = 0; row < stm.rowcount(); ++row)
    {
        cache_append(stm.get_int4(row, 0), stm.get_string(row, 1),
                     stm.get_string(row, 2), stm.get_int4(row, 3),
                     stm.get_string(row, 4), stm.get_int4(row, 5));
    }
...
{
    unsigned id =
        conn.exec_one_row("SELECT MAX(id) FROM repinfo").get_int4(0, 0);
    int prio =
        conn.exec_one_row("SELECT MAX(prio) FROM repinfo").get_int4(0, 0);
    ...
    conn.exec_no_data(R"(
        UPDATE repinfo SET memo=$2::text, description=$3::text, prio=$4::int4, descriptor=$5::text, tablea=$6::int4
         WHERE id=$1::int4
                      ...
                      (int32_t)entry.id, entry.new_memo, entry.new_desc,
                      (int32_t)entry.new_prio, entry.new_descriptor,
...
void PostgreSQLRepinfo::dump(FILE* out)
{
    ...
    for (unsigned row = 0; row < stm.rowcount(); ++row)
    {
        ...
        fprintf(out, " %4d   %s  %s  %d  %s %d\n", stm.get_int4(row, 0),
                memo.c_str(), desc.c_str(), stm.get_int4(row, 3),
                descriptor.c_str(), stm.get_int4(row, 5));
        ++count;
    };
