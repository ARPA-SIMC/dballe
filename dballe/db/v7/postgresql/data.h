...
template <typename Parent> class PostgreSQLDataCommon : public Parent
{
protected:
    /// DB connection
...
public:
    PostgreSQLDataCommon(v7::Transaction& tr,
                         dballe::sql::PostgreSQLConnection& conn);
    PostgreSQLDataCommon(const PostgreSQLDataCommon&)            = delete;
    PostgreSQLDataCommon(const PostgreSQLDataCommon&&)           = delete;
    PostgreSQLDataCommon& operator=(const PostgreSQLDataCommon&) = delete;

    void update(Tracer<>& trc, std::vector<typename Parent::BatchValue>& vars,
                bool with_attrs) override;
    void read_attrs(
...
public:
    ...
    PostgreSQLStationData(v7::Transaction& tr,
                          dballe::sql::PostgreSQLConnection& conn);

    void
    query(Tracer<>& trc, int id_station,
          std::function<void(int id, wreport::Varcode code)> dest) override;
    void insert(Tracer<>& trc, int id_station,
                std::vector<batch::StationDatum>& vars,
                bool with_attrs) override;
    void run_station_data_query(
...
public:
    ...
    PostgreSQLData(v7::Transaction& tr,
                   dballe::sql::PostgreSQLConnection& conn);

    void query(Tracer<>& trc, int id_station, const Datetime& datetime,
               std::function<void(int id, int id_levtr, wreport::Varcode code)>
                   dest) override;
    void insert(Tracer<>& trc, int id_station, const Datetime& datetime,
