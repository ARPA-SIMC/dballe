...
PostgreSQLStation::PostgreSQLStation(v7::Transaction& tr,
                                     PostgreSQLConnection& conn)
    : v7::Station(tr), conn(conn)
{
    // Precompile our statements
    conn.prepare("v7_station_select_fixed",
                 "SELECT id FROM station WHERE rep=$1::int4 AND lat=$2::int4 "
                 "AND lon=$3::int4 AND ident IS NULL");
    conn.prepare("v7_station_select_mobile",
                 "SELECT id FROM station WHERE rep=$1::int4 AND lat=$2::int4 "
                 "AND lon=$3::int4 AND ident=$4::text");
    conn.prepare(
        "v7_station_insert",
        "INSERT INTO station (id, rep, lat, lon, ident) VALUES (DEFAULT, "
        "$1::int4, $2::int4, $3::int4, $4::text) RETURNING id");
    conn.prepare("v7_station_select_station_data",
                 "SELECT rep, lat, lon, ident FROM station WHERE id=$1::int4");
...
DBStation PostgreSQLStation::lookup(Tracer<>& trc, int id_station)
{
    ...
    Result res(
        conn.exec_prepared("v7_station_select_station_data", id_station));
    if (trc)
        trc_sel.reset(trc->trace_select("v7_station_select_station_data",
                                        res.rowcount()));

    unsigned rows = res.rowcount();
    if (trc_sel)
        trc_sel->add_row(rows);
    switch (rows)
    {
        case 0: {
            stringstream msg;
        ...
        default:
            error_consistency::throwf(
                "select station data query returned %u results", rows);
    }
}
...
int PostgreSQLStation::maybe_get_id(Tracer<>& trc, const dballe::DBStation& st)
{
    ...
    if (st.ident.get())
    {
        if (trc)
            trc_sel.reset(
                trc->trace_select("v7_station_select_mobile", res.rowcount()));
        res = move(conn.exec_prepared("v7_station_select_mobile", rep,
                                      st.coords.lat, st.coords.lon,
                                      st.ident.get()));
    }
    else
    {
        if (trc)
            trc_sel.reset(trc->trace_select("v7_station_select_fixed"));
        res = move(conn.exec_prepared("v7_station_select_fixed", rep,
                                      st.coords.lat, st.coords.lon));
    }
    unsigned rows = res.rowcount();
    if (trc_sel)
    ...
    switch (rows)
    {
        ...
        default:
            error_consistency::throwf(
                "select station ID query returned %u results", rows);
    }
}
...
int PostgreSQLStation::insert_new(Tracer<>& trc, const dballe::DBStation& desc)
{
    ...
    return conn
        .exec_prepared_one_row("v7_station_insert", rep, desc.coords.lat,
                               desc.coords.lon, desc.ident.get())
        .get_int4(0, 0);
}

void PostgreSQLStation::get_station_vars(
    Tracer<>& trc, int id_station,
    std::function<void(std::unique_ptr<wreport::Var>)> dest)
{
    using namespace dballe::sql::postgresql;

    TRACE("get_station_vars Performing query v7_station_get_station_vars with "
          "idst %d\n",
          id_station);
    Tracer<> trc_sel(trc ? trc->trace_select("v7_station_get_station_vars")
                         : nullptr);
    ...
    for (unsigned row = 0; row < res.rowcount(); ++row)
    {
        ...
        if (!res.is_null(row, 2))
        {
            ...
            DBValues::decode(
                res.get_bytea(row, 2),
                [&](unique_ptr<wreport::Var> a) { var->seta(move(a)); });
        }
...
void PostgreSQLStation::add_station_vars(Tracer<>& trc, int id_station,
                                         DBValues& values)
{
    using namespace dballe::sql::postgresql;
    Tracer<> trc_sel(trc ? trc->trace_select("v7_station_add_station_vars")
                         : nullptr);
    ...
    for (unsigned row = 0; row < res.rowcount(); ++row)
        values.set(
            newvar((Varcode)res.get_int4(row, 0), res.get_string(row, 1)));
}

void PostgreSQLStation::run_station_query(
    ...
    std::function<void(int, int, const Coords& coords, const char* ident)> out)
{
    ...
    {
        const char* ident =
            res.is_null(row, 4) ? nullptr : res.get_string(row, 4);
        out(res.get_int4(row, 0), res.get_int4(row, 1),
            Coords((int)res.get_int4(row, 2), (int)res.get_int4(row, 3)),
            ident);
    }
}
