...
{
    if (enq.search_b_value(row().value))
        return;
...
void Summary::enq(impl::Enq& enq) const
{
    ...
    switch (key) // mklookup
    {
        ...
        case "station":  enq.set_station(row().station);
        case "datetimemax":
            if (row().dtrange.is_missing())
                return;
            else
                enq.set_datetime(row().dtrange.max);
        case "datetimemin":
            if (row().dtrange.is_missing())
                return;
            else
                enq.set_datetime(row().dtrange.min);
        case "yearmax":
            if (row().dtrange.is_missing())
                return;
            else
                enq.set_int(row().dtrange.max.year);
        case "yearmin":
            if (row().dtrange.is_missing())
                return;
            else
                enq.set_int(row().dtrange.min.year);
        case "monthmax":
            if (row().dtrange.is_missing())
                return;
            else
                enq.set_int(row().dtrange.max.month);
        case "monthmin":
            if (row().dtrange.is_missing())
                return;
            else
                enq.set_int(row().dtrange.min.month);
        case "daymax":
            if (row().dtrange.is_missing())
                return;
            else
                enq.set_int(row().dtrange.max.day);
        case "daymin":
            if (row().dtrange.is_missing())
                return;
            else
                enq.set_int(row().dtrange.min.day);
        case "hourmax":
            if (row().dtrange.is_missing())
                return;
            else
                enq.set_int(row().dtrange.max.hour);
        case "hourmin":
            if (row().dtrange.is_missing())
                return;
            else
                enq.set_int(row().dtrange.min.hour);
        case "minumax":
            if (row().dtrange.is_missing())
                return;
            else
                enq.set_int(row().dtrange.max.minute);
        case "minumin":
            if (row().dtrange.is_missing())
                return;
            else
                enq.set_int(row().dtrange.min.minute);
        case "secmax":
            if (row().dtrange.is_missing())
                return;
            else
                enq.set_int(row().dtrange.max.second);
        case "secmin":
            if (row().dtrange.is_missing())
                return;
            else
                enq.set_int(row().dtrange.min.second);
        case "level":      enq.set_level(get_levtr().level);
        case "leveltype1": enq.set_dballe_int(get_levtr().level.ltype1);
        case "l1":         enq.set_dballe_int(get_levtr().level.l1);
        case "leveltype2": enq.set_dballe_int(get_levtr().level.ltype2);
