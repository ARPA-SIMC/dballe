...
static const char* insert_query = "INSERT INTO station (rep, lat, lon, ident)"
                                  " VALUES (?, ?, ?, ?);";
...
DBStation SQLiteStation::lookup(Tracer<>& trc, int id_station)
{
    ...
    if (trc)
        trc_sel.reset(trc->trace_select(select_station_data_query));
...
int SQLiteStation::maybe_get_id(Tracer<>& trc, const dballe::DBStation& st)
{
    ...
    if (st.ident.get())
    {
        ...
        s = smstm;
        if (trc)
            trc_sel.reset(trc->trace_select(select_mobile_query));
    }
    else
    {
        ...
        s = sfstm;
        if (trc)
            trc_sel.reset(trc->trace_select(select_fixed_query));
    }
    bool found = false;
    int id;
    s->execute_one([&]() {
...
int SQLiteStation::insert_new(Tracer<>& trc, const dballe::DBStation& desc)
{
    ...
    istm->execute();
    if (trc)
        trc->trace_insert(insert_query, 1);
    return conn.get_last_insert_id();
}

void SQLiteStation::get_station_vars(
...
void SQLiteStation::add_station_vars(Tracer<>& trc, int id_station,
                                     DBValues& values)
{
    const char* query = R"(
    ...
    stm->execute([&]() {
        if (trc_sel)
            trc_sel->add_row();
        values.set(newvar((wreport::Varcode)stm->column_int(0),
                          stm->column_string(1)));
    });
}

void SQLiteStation::run_station_query(
...
void SQLiteStation::_dump(
    std::function<void(int, int, const Coords& coords, const char* ident)> out)
{
    auto stm =
        conn.sqlitestatement("SELECT id, rep, lat, lon, ident FROM station");
    stm->execute([&]() {
        const char* ident =
            stm->column_isnull(4) ? nullptr : stm->column_string(4);
        out(stm->column_int(0), stm->column_int(1),
            Coords(stm->column_int(2), stm->column_int(3)), ident);
    });
}
