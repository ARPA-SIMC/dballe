...
struct Fixture : dballe::tests::DriverFixture
{
    ...
    void reset_context()
    {
        if (conn->has_table("context"))
            driver->exec_no_data("DELETE FROM context");
        if (auto c = dynamic_cast<ODBCConnection*>(conn))
            co.reset(new Context(*c));
        else
...
test_group tg("db_sql_context_v5_odbc", "ODBC", db::V5, tests);
