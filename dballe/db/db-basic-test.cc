...
template <typename DB> void Tests<DB>::register_tests()
{
    ...
    this->add_method("stationdata", [](Fixture& f) {
        ...
        switch (DB::format)
        {
            case Format::V7: {
                ...
                for (unsigned i = 0; i < 2; ++i)
                {
                    ...
                    if (station.report == "temp")
                    {
                        ...
                        wassert(actual(values.var(WR_VAR(0, 1, 19))) ==
                                "Esmac");
                        have_temp = true;
                    }
            ...
            default:
                throw error_unimplemented(
                    "testing stations_without_data on unsupported database");
        }
        ...
        wassert(
            actual(
                impl::Message::downcast(msgs[0])->get_rep_memo_var()->enqc()) ==
            "synop");
        wassert(
            actual(
                impl::Message::downcast(msgs[0])->get_st_name_var()->enqc()) ==
            "Camse");
        wassert(
            actual(
                impl::Message::downcast(msgs[0])->get_temp_2m_var()->enqd()) ==
            273.15);
        wassert(
            actual(
                impl::Message::downcast(msgs[1])->get_rep_memo_var()->enqc()) ==
            "temp");
        wassert(
            actual(
                impl::Message::downcast(msgs[1])->get_st_name_var()->enqc()) ==
            "Esmac");
        wassert(
            actual(
                impl::Message::downcast(msgs[1])->get_temp_2m_var()->enqd()) ==
            274.15);
    });
...
template <typename DB> void CommitTests<DB>::register_tests()
{
    ...
    this->add_method("wipe", [](Fixture& f) {
        ...
        wassert(actual(db->query_station_data(core::Query())->remaining()) ==
                0u);
    ...
    this->add_method("transaction_create_error", [](Fixture& f) {
        ...
        catch (dballe::error_db& e)
        {
            wassert(actual(e.what()).matches(
                "^cannot compile query|relation \"repinfo\" does not "
                "exist|Table 'test\\.repinfo' doesn't exist"));
        }
    ...
    this->add_method("transactions_after_fork", [](Fixture& f) {
        ...
        protected:
            ...
            int main() noexcept override
            {
                try
                {
                    ...
                    fprintf(stderr, "Opening a transaction in forked process "
                ...
                catch (std::exception& e)
                {
                    ...
                    if (msg.find("database connections cannot be used after "
                                 "forking") != std::string::npos)
                        return 0;
                    fprintf(stderr,
                            "Unexpected error starting a transaction in test "
                            "child: %s",
                            e.what());
                    return 2;
                }
        ...
        public:
            TestChild(std::shared_ptr<dballe::DB> db) : db(db) {}
        };
