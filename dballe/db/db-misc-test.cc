...
unsigned run_attr_query_data(std::shared_ptr<db::Transaction> tr, int data_id,
                             Values& dest)
{
    unsigned count = 0;
    tr->attr_query_data(data_id, [&](unique_ptr<Var> var) {
        dest.set(move(var));
        ++count;
    });
    return count;
...
template <typename DB> void Tests<DB>::register_tests()
{
    ...
    this->add_method("insert", [](Fixture& f) {
        ...
        wassert(f.tr->attr_insert_data(
            ds.data["synop"].values.value(WR_VAR(0, 12, 101)).data_id, attrs));
        ...
        // Query and verify the station data
        {
            ...
            cur->next();
            wassert(
                actual(cur).station_keys_match(ds.stations["synop"].station));
        ...
        // Query and verify the measured data
        {
            ...
            wassert(actual(cur).data_var_matches(ds.data["synop"],
                                                 WR_VAR(0, 12, 101)));
        }
        ...
        // Query and verify attributes
        {
            int count = 0;
            unique_ptr<Var> attr;
            wassert(f.tr->attr_query_data(
                ds.data["synop"].values.value(WR_VAR(0, 12, 101)).data_id,
                [&](std::unique_ptr<wreport::Var>&& var) noexcept {
                    ++count;
                    attr = move(var);
                }));
    ...
    this->add_method("insert_perms", [](Fixture& f) {
        ...
        // Check if adding a nonexisting station when not allowed causes an
        // error
        try
        {
        ...
        catch (error_consistency& e)
        {
            wassert(actual(e.what()).contains(
                "insert a station entry when it is forbidden"));
        }
        ...
        wassert(actual(oldf.data["synop"].values.value("B01011").data_id) ==
                MISSING_INT);
        wassert(actual(oldf.data["synop"].values.value("B01012").data_id) ==
                MISSING_INT);
        ...
        catch (wreport::error& e)
        {
            wassert(actual(e.what()).matches(
                "refusing to overwrite existing data|cannot replace an "
                "existing value|Duplicate entry"));
        }
    });
    ...
    this->add_method("query_station", [](Fixture& f) {
        ...
        switch (DB::format)
        {
            case Format::V7: {
                ...
                // Memdb and V7 have one station entry per (lat, lon, ident,
                // network)
                ...
                for (unsigned i = 0; i < 2; ++i)
                {
                    ...
                    {
                        wassert(actual(station.coords) ==
                                Coords(12.34560, 76.54320));
                        wassert(actual(station.ident.is_missing()).istrue());
                        wassert(actual(cur).station_keys_match(
                            oldf.stations["synop"].station));
                        have_synop = true;
                    }
    ...
    this->add_method("delete", [](Fixture& f) {
        ...
        for (unsigned i = 0; i < 2; ++i)
        {
            ...
            switch (last_code)
            {
                case WR_VAR(0, 1, 11):
                case WR_VAR(0, 1, 12):
                    wassert(actual(cur).data_var_matches(oldf.data["synop"],
                                                         last_code));
                    break;
                default:
                    throw TestFailed("got a varcode that we did not ask for: " +
                                     varcode_format(last_code));
            }
...
#define WANTRESULT(querystr, ab)                                               \
    do                                                                         \
    {                                                                          \
        auto cur =                                                             \
            f.tr->query_data(*dballe::tests::query_from_string(querystr));     \
        wassert(actual(cur->remaining()) == 1);                                \
        wassert(actual(cur->next()).istrue());                                 \
        wassert(actual(cur->remaining()) == 0);                                \
    ...
    this->add_method("attrs", [](Fixture& f) {
        ...
        while (cur->next())
        {
            ...
            {
                context_id = dynamic_cast<db::CursorData*>(cur.get())
                                 ->attr_reference_id();
                cur->discard();
                found = true;
                break;
    ...
    this->add_method("attrs1", [](Fixture& f) {
        ...
        qc.set("B07025", 9);
        qc.set("B05022", 10);
        f.tr->attr_insert_data(
            oldf.data["synop"].values.value(WR_VAR(0, 1, 11)).data_id, qc);
        ...
        cur->next();
        int attr_id =
            dynamic_cast<db::CursorData*>(cur.get())->attr_reference_id();
        cur->discard();
        ...
        wassert(actual_varcode(qc.var("B01007").code()) == WR_VAR(0, 1, 7));
        wassert(actual(qc.var("B01007")) == 1);
        wassert(actual_varcode(qc.var("B02048").code()) == WR_VAR(0, 2, 48));
        wassert(actual(qc.var("B02048")) == 2);
        wassert(actual_varcode(qc.var("B05021").code()) == WR_VAR(0, 5, 21));
        wassert(actual(qc.var("B05021")) == 8);
        wassert(actual_varcode(qc.var("B05022").code()) == WR_VAR(0, 5, 22));
        wassert(actual(qc.var("B05022")) == 10);
        wassert(actual_varcode(qc.var("B05040").code()) == WR_VAR(0, 5, 40));
        wassert(actual(qc.var("B05040")) == 3);
        wassert(actual_varcode(qc.var("B05041").code()) == WR_VAR(0, 5, 41));
        wassert(actual(qc.var("B05041")) == 4);
        wassert(actual_varcode(qc.var("B05043").code()) == WR_VAR(0, 5, 43));
        wassert(actual(qc.var("B05043")) == 5);
        wassert(actual_varcode(qc.var("B07024").code()) == WR_VAR(0, 7, 24));
        wassert(actual(qc.var("B07024")) == 7);
        wassert(actual_varcode(qc.var("B07025").code()) == WR_VAR(0, 7, 25));
        wassert(actual(qc.var("B07025")) == 9);
        wassert(actual_varcode(qc.var("B33032").code()) == WR_VAR(0, 33, 32));
        wassert(actual(qc.var("B33032")) == 6);
    });
    this->add_method("longitude_wrap", [](Fixture& f) {
        ...
        auto cur = f.tr->query_data(*query_from_string(
            "latmin=10.0, latmax=15.0, lonmin=70.0, lonmax=-160.0"));
        wassert(actual(cur->remaining()) == 2);
        cur->discard();
    });
    this->add_method("query_ana_filter", [](Fixture& f) {
        ...
        auto cur =
            f.tr->query_data(*query_from_string("rep_memo=metar, var=B01011"));
        wassert(actual(cur->remaining()) == 1);
        ...
        cur = f.tr->query_data(*query_from_string(
            "rep_memo=metar, var=B01011, attr_filter=B01001=50"));
        wassert(actual(cur->remaining()) == 1);
        cur->discard();
        ...
        cur = f.tr->query_data(*query_from_string(
            "rep_memo=metar, var=B01011, attr_filter=B01001<51"));
        wassert(actual(cur->remaining()) == 1);
        cur->discard();

        cur = f.tr->query_data(*query_from_string(
            "rep_memo=metar, var=B01011, attr_filter=B01001<8"));
        wassert(actual(cur->remaining()) == 0);
        cur->discard();

        // Try queries filtered by string attributes
        cur = f.tr->query_data(*query_from_string(
            "rep_memo=metar, var=B01011, attr_filter=B01008=50"));
        wassert(actual(cur->remaining()) == 1);
        cur->discard();
        ...
        cur = f.tr->query_data(*query_from_string(
            "rep_memo=metar, var=B01011, attr_filter=B01008<8"));
        wassert(actual(cur->remaining()) == 1);
        cur->discard();

        cur = f.tr->query_data(*query_from_string(
            "rep_memo=metar, var=B01011, attr_filter=B01008<100"));
        wassert(actual(cur->remaining()) == 0);
        cur->discard();
    });
    this->add_method("query_station_best", [](Fixture& f) {
        ...
        core::Query q;
        q.dtrange = DatetimeRange(Datetime(1000, 1, 1, 0, 0, 0),
    ...
    this->add_method("query_best_bug1", [](Fixture& f) {
        // Reproduce a querybest scenario which produced always the same data
        // record
        ...
        // Query all with best
        auto cur =
            f.tr->query_data(*query_from_string("var=B12101, query=best"));
    ...
    this->add_method("update", [](Fixture& f) {
        ...
        q.dtrange = DatetimeRange(Datetime(1945, 4, 25, 8, 0, 0),
                                  Datetime(1945, 4, 25, 8, 0, 0));
        q.report  = "synop";
        ...
        {
            case Format::V7:
                wassert(actual(run_attr_query_data(
                            f.tr,
                            dynamic_cast<db::CursorData*>(cur.get())
                                ->attr_reference_id(),
                            qattrs)) == 0);
                break;
            default:
                throw TestFailed("Database format " +
                                 to_string((int)DB::format) + " not supported");
        }
    });
    this->add_method("query_stepbystep", [](Fixture& f) {
        ...
        // just call to_record now, to check if in the next call old variables
        // are removed
    ...
    this->add_method("insert_stationinfo_twice1", [](Fixture& f) {
        ...
        // Use a sorted vector because while all DBs group by report, not all
        // DBs sort by report name.
    ...
    this->add_method("query_bad_attrfilter", [](Fixture& f) {
        ...
        {
            wassert(actual(e.what()).matches(
                "B12001 is not a valid filter|cannot find any operator in "
                "filter 'B12001'"));
        }
    });
    this->add_method("query_best_priomax", [](Fixture& f) {
        ...
        static const char* rep_memos[] = {
            "synop",     "metar",     "temp",    "pilot", "buoy",
            "ship",      "tempship",  "airep",   "amdar", "acars",
            "pollution", "satellite", "generic", NULL};
        ...
        // Query with querybest only
        {
            core::Query query;
            query.query   = "best";
            query.dtrange = DatetimeRange(Datetime(2009, 11, 11, 0, 0, 0),
                                          Datetime(2009, 11, 11, 0, 0, 0));
            query.varcodes.insert(WR_VAR(0, 12, 101));
            auto cur = f.tr->query_data(query);
            wassert(actual(cur->remaining()) == 1);
            wassert(actual(cur->next()).istrue());
