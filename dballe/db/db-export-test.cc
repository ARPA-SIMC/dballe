...
template <typename DB> void Tests<DB>::register_tests()
{
    ...
    this->add_method("export", [](Fixture& f) {
        ...
        // Put some data in the database and check that it gets exported
        // properly Query back the data
        impl::Messages messages =
            dballe::tests::messages_from_db(f.tr, core::Query());
        ...
        wassert(actual(impl::Message::downcast(messages[0])->type) ==
                MessageType::SYNOP);
        ...
        wassert(actual(messages[0]->get_datetime()) ==
                Datetime(1945, 4, 25, 8, 0, 0));
        wassert(actual_var(*messages[0], WR_VAR(0, 1, 12), Level(1, 2, 0, 3),
                           Trange(4, 5, 6)) == 500);

        wassert(actual(impl::Message::downcast(messages[1])->type) ==
                MessageType::SYNOP);
        ...
        wassert(actual(messages[1]->get_datetime()) ==
                Datetime(1945, 4, 26, 8, 0, 0));
        wassert(actual_var(*messages[1], WR_VAR(0, 1, 12), Level(1, 2, 0, 3),
                           Trange(4, 5, 6)) == 400);

        wassert(actual(impl::Message::downcast(messages[synmsg])->type) ==
                MessageType::SYNOP);
        ...
        wassert(actual(messages[synmsg]->get_datetime()) ==
                Datetime(1945, 4, 26, 8, 0, 0));
        wassert(actual_var(*messages[synmsg], WR_VAR(0, 1, 12),
                           Level(1, 2, 0, 3), Trange(4, 5, 6)) == 300);

        wassert(actual(impl::Message::downcast(messages[metmsg])->type) ==
                MessageType::METAR);
        ...
        wassert(actual(messages[metmsg]->get_datetime()) ==
                Datetime(1945, 4, 26, 8, 0, 0));
        wassert(actual_var(*messages[metmsg], WR_VAR(0, 1, 12),
                           Level(1, 2, 0, 3), Trange(4, 5, 6)) == 200);
    });
    this->add_method("export", [](Fixture& f) {
    ...
    this->add_method("missing_repmemo", [](Fixture& f) {
        ...
        impl::Messages msgs =
            wcallchecked(dballe::tests::messages_from_db(f.tr, query));
