...
template <typename Station> BaseSummary<Station>::BaseSummary() {}

template <typename Station> BaseSummary<Station>::~BaseSummary() {}

template <typename Station>
std::shared_ptr<dballe::CursorSummary>
BaseSummary<Station>::query_summary(const Query& query) const
{
...
{
    add(cur.get_station(),
        summary::VarDesc(cur.get_level(), cur.get_trange(), cur.get_varcode()),
        cur.get_datetimerange(), cur.get_count());
}
                                       ...
                                       bool station_data, bool data)
{
    ...
    if (station.coords.is_missing())
        throw wreport::error_notfound(
            "coordinates not found in message to summarise");
...
template <typename Station>
void BaseSummary<Station>::add_messages(
...
template <typename Station>
void BaseSummary<Station>::add_filtered(const BaseSummary<Station>& summary,
...
template <typename Station>
void BaseSummary<Station>::add_summary(
    const BaseSummary<dballe::Station>& summary)
{
    summary.iter([&](const dballe::Station& station,
                     const summary::VarDesc& var, const DatetimeRange& dtrange,
                     size_t count) {
        add(summary::convert_station<Station, dballe::Station>(station), var,
            dtrange, count);
        return true;
    });
}
...
{
    summary.iter([&](const dballe::DBStation& station,
                     const summary::VarDesc& var, const DatetimeRange& dtrange,
                     size_t count) {
        add(summary::convert_station<Station, dballe::DBStation>(station), var,
            dtrange, count);
        return true;
    });
}
...
// internal compiler error on centos7:
// https://travis-ci.org/ARPA-SIMC/dballe/jobs/648366650#L1906
template <typename Station> struct Loader
{
    BaseSummary<Station>& summary;
    Station station;
    std::vector<summary::VarEntry> vars;

    Loader(BaseSummary<Station>& summary) : summary(summary) {}
