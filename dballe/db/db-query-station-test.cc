...
class Tests : public FixtureTestCase<TransactionFixture<DB, DBData>>
{
    ...
    void register_tests() override
    {
        ...
        this->add_method("query_lat_lon", [](Fixture& f) {
            ...
            wassert(
                actual(f.tr).try_station_query("lonmin=10., lonmax=20.", 0));
            wassert(actual(f.tr).try_station_query(
                "lonmin=76.54320, lonmax=76.54320", 2));
            wassert(actual(f.tr).try_station_query(
                "lonmin=76.54320, lonmax=77.", 2));
            wassert(actual(f.tr).try_station_query(
                "lonmin=76.54330, lonmax=77.", 0));
            wassert(
                actual(f.tr).try_station_query("lonmin=60., lonmax=77.", 4));
            wassert(actual(f.tr).try_station_query(
                "lonmin=77., lonmax=76.54310", 2));
            wassert(actual(f.tr).try_station_query(
                "lonmin=77., lonmax=76.54320", 4));
            wassert(
                actual(f.tr).try_station_query("lonmin=77., lonmax=-10", 0));
        });
        ...
        this->add_method("query_ana_filter", [](Fixture& f) {
            ...
            wassert(actual(f.tr).try_station_query("ana_filter=50<=B07030<=100",
                                                   2));
        });
        this->add_method("query_var", [](Fixture& f) {
            /*
             * Querying var= or varlist= on a station query means querying
             * stations that measure that variable or those variables.
             */
...
#warning TODO: fix this test to give an error once we do not need to support this bug anymore
        ...
        this->add_method("query_ordering", [](Fixture& f) {
            ...
            switch (f.db->format())
            {
                case Format::V7: wassert(actual(cur->remaining()) == 4); break;
                default:
                    error_unimplemented::throwf(
                        "cannot run this test on a database of format %d",
                        (int)DB::format);
