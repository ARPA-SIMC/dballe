...
static dba_err copy_ana_in_sea(dba_msg msg, dba_record rec)
{
	int rep_cod;
...
dba_err dba_db_export_sea(dba db, dba_msg** msgs, dba_record query)
{
	...
	if (mlist == NULL)
	{
		err = dba_error_alloc("allocating space for an array of seas");
		goto cleanup;
	}
	i = 0;
...
//				dba_unset(msg->context, "pindicator"); dba_unset(msg->context, "p1"); dba_unset(msg->context, "p2");
			...
			DBA_RUN_OR_GOTO(cleanup, copy_ana_in_sea(msg, res));
