...
static dba_err copy_ana_in_synop(dba_msg msg, dba_record rec)
{
	msg->type = MSG_SYNOP;
...
dba_err dba_db_export_synop(dba db, dba_msg** msgs, dba_record query)
{
	...
	if (mlist == NULL)
	{
		err = dba_error_alloc("allocating space for an array of synops");
		goto cleanup;
	}
	i = 0;
	...
	do {
		...
		if (lat != last_lat || lon != last_lon || strcmp(datetime, last_datetime) != 0)
		{
			...
			DBA_RUN_OR_GOTO(cleanup, copy_ana_in_synop(msg, res));
