...
template <typename Station> void Cursor<Station>::enq(impl::Enq& enq) const
{
    ...
    switch (key) // mklookup
    {
        ...
        case "ana_id":
            enq.set_dballe_int(get_station_id(station_entry->station));
        ...
        case "datetimemax":
            if (var_entry->dtrange.is_missing())
                return;
            else
                enq.set_datetime(var_entry->dtrange.max);
        case "datetimemin":
            if (var_entry->dtrange.is_missing())
                return;
            else
                enq.set_datetime(var_entry->dtrange.min);
        case "yearmax":
            if (var_entry->dtrange.is_missing())
                return;
            else
                enq.set_int(var_entry->dtrange.max.year);
        case "yearmin":
            if (var_entry->dtrange.is_missing())
                return;
            else
                enq.set_int(var_entry->dtrange.min.year);
        case "monthmax":
            if (var_entry->dtrange.is_missing())
                return;
            else
                enq.set_int(var_entry->dtrange.max.month);
        case "monthmin":
            if (var_entry->dtrange.is_missing())
                return;
            else
                enq.set_int(var_entry->dtrange.min.month);
        case "daymax":
            if (var_entry->dtrange.is_missing())
                return;
            else
                enq.set_int(var_entry->dtrange.max.day);
        case "daymin":
            if (var_entry->dtrange.is_missing())
                return;
            else
                enq.set_int(var_entry->dtrange.min.day);
        case "hourmax":
            if (var_entry->dtrange.is_missing())
                return;
            else
                enq.set_int(var_entry->dtrange.max.hour);
        case "hourmin":
            if (var_entry->dtrange.is_missing())
                return;
            else
                enq.set_int(var_entry->dtrange.min.hour);
        case "minumax":
            if (var_entry->dtrange.is_missing())
                return;
            else
                enq.set_int(var_entry->dtrange.max.minute);
        case "minumin":
            if (var_entry->dtrange.is_missing())
                return;
            else
                enq.set_int(var_entry->dtrange.min.minute);
        case "secmax":
            if (var_entry->dtrange.is_missing())
                return;
            else
                enq.set_int(var_entry->dtrange.max.second);
        case "secmin":
            if (var_entry->dtrange.is_missing())
                return;
            else
                enq.set_int(var_entry->dtrange.min.second);
        ...
        default:
            wreport::error_notfound::throwf(
                "key %s not found on this query result", key);
    }
}
