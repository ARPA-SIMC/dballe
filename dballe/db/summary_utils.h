...
struct VarEntry
{
    ...
    VarEntry(const VarDesc& var, const dballe::DatetimeRange& dtrange,
             size_t count)
    ...
    {
        return std::tie(var, dtrange, count) ==
               std::tie(o.var, o.dtrange, o.count);
    }
    bool operator!=(const VarEntry& o) const
    {
        return std::tie(var, dtrange, count) !=
...
inline const VarDesc& station_entry_get_value(const VarEntry& item)
{
    return item.var;
}
    ...
    : protected core::SmallSet<VarEntry, VarDesc, station_entry_get_value>
{
    ...
    StationEntry(const Station& station, const VarDesc& vd,
                 const dballe::DatetimeRange& dtrange, size_t count)
        : station(station)
    {
    ...
    void add(const VarDesc& vd, const dballe::DatetimeRange& dtrange,
             size_t count);
    template <typename OStation>
...
station_entries_get_value(const StationEntry<Station>& item)
{
    return item.station;
}
 ...
 */
template <typename Station>
struct StationEntries
    : protected core::SmallSet<StationEntry<Station>, Station,
                               station_entries_get_value<Station>>
{
    typedef core::SmallSet<StationEntry<Station>, Station,
                           station_entries_get_value<Station>>
        Parent;
    ...
    void add(const Station& station, const VarDesc& vd,
             const dballe::DatetimeRange& dtrange, size_t count);
...
template <typename S1, typename S2> inline S1 convert_station(const S2& s)
{
    throw wreport::error_unimplemented("unsupported station conversion");
}

template <> inline Station convert_station<Station, Station>(const Station& s)
{
    return s;
}
template <>
inline DBStation convert_station<DBStation, DBStation>(const DBStation& s)
{
    return s;
}
...
struct StationFilterBase
{
        ...
        : q(core::Query::downcast(query))
    {
        ...
        // If there is any filtering on the station, build a whitelist of
        // matching stations
    ...
    template <typename Station> bool matches_station(const Station& station)
    {
        if (has_flt_area)
        {
...
template <class Station> struct StationFilter;

template <> struct StationFilter<dballe::Station> : public StationFilterBase
{
...
template <> struct StationFilter<dballe::DBStation> : public StationFilterBase
{
    StationFilter(const dballe::Query& query) : StationFilterBase(query)
    {
...
template <typename Station> struct Cursor : public impl::CursorSummary
{
    ...
    int remaining() const override { return _remaining; }
    ...
    void discard() override { station_entry = results.end(); }
    ...
    {
        return var_entry->var.varcode;
    }
    DatetimeRange get_datetimerange() const override
    {
        return var_entry->dtrange;
    }
