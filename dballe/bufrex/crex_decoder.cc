...
struct Decoder 
{
	...
	Decoder(const Rawmsg& in, CrexMsg& out)
	...
	void parse_error(const char* fmt, ...) THROWF_ATTRS(2, 3)
	{
		...
		asprintf(&context, "%s:%d+%d: %s", filename(), in.offset, cur - start, message);
	...
	void decode_header()
	{
		...
		IFTRACE{
			...
			for (vector<Varcode>::const_iterator i = out.datadesc.begin();
				...
				TRACE(" %01d%02d%03d", DBA_VAR_F(*i), DBA_VAR_X(*i), DBA_VAR_Y(*i));
			TRACE("\n");
		}
	}
...
unsigned Decoder::parse_r_data(const Opcodes& ops)
{
	...
	if (count == 0)
	{
		...
		current_subset->store_variable_i(DBA_VAR(0, 31, 1), count);
