...
struct Decoder 
{
	...
	Decoder(const Rawmsg& in, BufrMsg& out)
	...
	void parse_error(const unsigned char* pos, const char* fmt, ...) THROWF_ATTRS(3, 4)
	{
		char* context;
		char* message;
		...
		asprintf(&context, "%s:%d+%d: %s", filename(), in.offset, pos - start, message);
...
struct opcode_interpreter
{
	...
	void parse_error(const char* fmt, ...) THROWF_ATTRS(2, 3)
	{
		char* context;
		char* message;
		...
		asprintf(&context, "%s:%d+%d: %s", filename(), d.in.offset, cursor, message);
		free(message);
	...
	void bitmap_next()
	{
			...
			(bitmap_use_index < 0 || bitmap->value()[bitmap_use_index] == '-'))
		{
			...
			while (bitmap_subset_index < d.out.subsets[0].size() &&
				DBA_VAR_F(d.out.subsets[0][bitmap_subset_index].code()) != 0)
				++bitmap_subset_index;
		}
	...
	{
		if (bitmap && DBA_VAR_X(var.code()) == 33)
		{
			TRACE("Adding var %01d%02d%03d %s as attribute to %01d%02d%03d bsi %d/%d\n",
					...
					DBA_VAR_Y(var.code()),
					var.value(),
					DBA_VAR_F(subset[bitmap_subset_index].code()),
					DBA_VAR_X(subset[bitmap_subset_index].code()),
					DBA_VAR_Y(subset[bitmap_subset_index].code()),
...
void Decoder::decode_data()
{
	...
	// Once we filled the Msg header info, load decoding tables
	out.load_tables();
...
{
	unsigned used = 1;
	group = DBA_VAR_X(ops.head());
	count = DBA_VAR_Y(ops.head());
