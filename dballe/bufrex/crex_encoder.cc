...
struct Encoder
{
	...
	Encoder(const CrexMsg& in, Rawmsg& out)
		: in(in), out(out),
	...
	void encode_sec1()
	{
				...
				i != in.datadesc.end(); ++i)
		{
			...
			if (DBA_VAR_F(*i) == 0 && DBA_VAR_X(*i) == 31 && DBA_VAR_Y(*i) < 3)
				continue;

			raw_appendf(" %c%02d%03d", prefix, DBA_VAR_X(*i), DBA_VAR_Y(*i));
		}
...
void CrexMsg::encode(Rawmsg& out) const
{
	Encoder e(*this, out);
	e.run();
	out.encoding = CREX;
