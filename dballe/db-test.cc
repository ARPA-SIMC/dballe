...
void Tests::register_tests()
{
    ...
    add_method("dbconnectoptions", [] {
        ...
        opts = DBConnectOptions::create(
            "postgresql:///testuser@testhost/testdb?port=5433");
        wassert(actual(opts->url) ==
                "postgresql:///testuser@testhost/testdb?port=5433");
        wassert_false(opts->wipe);
        opts->reset_actions();
        wassert_false(opts->wipe);
        ...
        opts = DBConnectOptions::create(
            "mysql://host/db?user=testuser&password=testpass");
        wassert(actual(opts->url) ==
                "mysql://host/db?user=testuser&password=testpass");
        wassert_false(opts->wipe);
        opts->reset_actions();
        ...
        opts = DBConnectOptions::create(
            "postgresql:///testuser@testhost/testdb?port=5433&wipe=yes");
        wassert(actual(opts->url) ==
                "postgresql:///testuser@testhost/testdb?port=5433");
        wassert_true(opts->wipe);
        opts->reset_actions();
        ...
        opts = DBConnectOptions::create(
            "mysql://host/db?wipe=yes&user=testuser&password=testpass");
        wassert(actual(opts->url) ==
                "mysql://host/db?user=testuser&password=testpass");
        wassert_true(opts->wipe);
        opts->reset_actions();
