...
template <typename DB> class Tests : public FixtureTestCase<DBFixture<DB>>
{
...
template <typename DB> void Tests<DB>::register_tests()
{
    ...
    this->add_method("import", [](Fixture& f) {
        ...
        wassert(actual(dbadb.do_import(
                    dballe::tests::datafile("bufr/obs0-1.22.bufr"), reader,
                    DBImportOptions::defaults)) == 0);
        ...
        impl::Messages msgs = importer->from_binary(file.msgs[0]);
    ...
    this->add_method("issue62", [](Fixture& f) {
        ...
        cmdline::Reader reader(opts);
        wassert(
            actual(dbadb.do_import(dballe::tests::datafile("bufr/issue62.bufr"),
                                   reader, DBImportOptions::defaults)) == 0);
