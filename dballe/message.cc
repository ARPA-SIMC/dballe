...
const wreport::Var* Message::get(const Level& lev, const Trange& tr,
...
void Message::set(const Level& lev, const Trange& tr, wreport::Varcode code,
...
void Message::set(const Level& lev, const Trange& tr,
...
void Message::set(const char* shortcut, std::unique_ptr<wreport::Var> var)
{
    const impl::Shortcut& s = impl::Shortcut::by_name(shortcut);
    if (s.code != var->code())
        error_consistency::throwf("Message::set(%s) called with varcode of "
                                  "%01d%02d%03d instead of %02d%02d%03d",
                                  shortcut, WR_VAR_FXY(var->code()),
                                  WR_VAR_FXY(s.code));
