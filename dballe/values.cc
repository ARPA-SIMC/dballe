...
template <typename Value>
typename ValuesBase<Value>::iterator
ValuesBase<Value>::find(wreport::Varcode code) noexcept
{
    /* Binary search */
    if (m_values.empty())
        return m_values.end();
...
template <typename Value>
typename ValuesBase<Value>::const_iterator
ValuesBase<Value>::find(wreport::Varcode code) const noexcept
{
    /* Binary search */
    if (m_values.empty())
        return m_values.end();
...
template <typename Value> void ValuesBase<Value>::unset(wreport::Varcode code)
{
...
template <typename Value> void ValuesBase<Value>::set(Value&& val)
{
...
template <typename Value> void ValuesBase<Value>::set(const wreport::Var& v)
{
    auto i = find(v.code());
    if (i == end())
        insert_new(Value(v));
...
const wreport::Var& ValuesBase<Value>::var(wreport::Varcode code) const
{
    ...
    if (!i->get())
        error_notfound::throwf("variable %01d%02d%03d not set", WR_VAR_F(code),
...
template <typename Value> void ValuesBase<Value>::print(FILE* out) const
{
    for (const auto& val : *this)
        val.print(out);
}

template <typename Value> std::vector<uint8_t> ValuesBase<Value>::encode() const
{
