...
void Tests::register_tests()
{
    ...
    add_method("domain_throw", []() {
        auto file              = File::create(Encoding::JSON,
                                              tests::datafile("json/issue241.json"), "r");
        ...
        wassert_true(file->foreach ([&](const BinaryMessage& bmsg) {
            return importer->foreach_decoded(
                bmsg, [&](std::shared_ptr<Message> dest) {
                    // Would throw: wreport::error_domain Value -30 is outside
                    // the range [-20,1048554] for 013013 (TOTAL SNOW DEPTH)
                    const wreport::Var* val = dest->get(
                        Level(1), Trange::instant(), WR_VAR(0, 13, 13));
                    wassert_true(val);
