...
void Tests::register_tests()
{
    ...
    add_bufr_method("synop-cloudbelow.bufr", [](const impl::Messages& msgs) {
        ...
        IS2(WR_VAR(0, 10, 51), Level(102, 490000), Trange::instant(),
            101060.0); // press_msl
        IS2(WR_VAR(0, 10, 63), Level(102, 490000), Trange(205, 0, 10800),
            6.0); // press_tend
    ...
    add_bufr_simplified_method(
        "temp-2-255.bufr", [](const impl::Messages& msgs) {
            ...
            wassert(actual(msg.find_context(Level(100, -1), Trange::instant()))
                        .isfalse());
        });
    ...
    add_method("outofrange", [] {
...

        {
            ...
            impl::Messages msgs =
                read_msgs("bufr/interpreted-range.bufr", Encoding::BUFR, *opts);
            wassert(actual(msgs.size()) == 1u);
            const impl::Message& msg = impl::Message::downcast(*msgs[0]);
    ...
    add_bufr_method("gts-synop-linate.bufr", [](const impl::Messages& msgs) {
        ...
        const impl::Message& msg = impl::Message::downcast(*msgs[0]);
        const Var* v =
            msg.get(Level(103, 2000), Trange(3, 0, 43200), WR_VAR(0, 12, 101));
    ...
    add_bufr_simplified_method(
        "test-soil1.bufr", [](const impl::Messages& msgs) {
            wassert(actual(msgs.size()) == 1u);
            const impl::Message& msg = impl::Message::downcast(*msgs[0]);
            wassert(actual(msg.type) == MessageType::SYNOP);
