...
class Tests : public TestCase
{
    ...
    void add_bufr_method(const std::string& fname,
                         std::function<void(const impl::Messages& msgs)> m)
    {
        add_method("prec_" + fname, [=]() {
            ...
            impl::Messages msgs =
                wcallchecked(read_msgs(pathname.c_str(), Encoding::BUFR, opts));
            m(msgs);
        });
    }

    void add_bufr_simplified_method(
        ...
        std::function<void(const impl::Messages& msgs)> m)
    {
        add_method("simp_" + fname, [=]() {
            ...
            impl::Messages msgs =
                read_msgs(pathname.c_str(), Encoding::BUFR, opts);
            m(msgs);
        });
    }

    void add_crex_method(const std::string& fname,
                         std::function<void(const impl::Messages& msgs)> m)
    {
        add_method(fname, [=]() {
    ...
    void register_tests() override
    {
        ...
        add_method("all_bufr", []() {
            ...
            for (int i = 0; files[i] != NULL; i++)
            {
                ...
                catch (std::exception& e)
                {
                    ...
                    {
                        BinaryMessage raw =
                            read_rawmsg(files[i], Encoding::BUFR);
                        unique_ptr<Bulletin> bulletin(
                            BufrBulletin::decode(raw.data));
                        bulletin->print(stderr);
                    }
                    catch (std::exception& e1)
                    {
                        cerr << "Cannot display failing bulletin: " << e1.what()
                             << endl;
                    }
        ...
        add_method("all_crex", []() {
            ...
            for (int i = 0; files[i] != NULL; i++)
            {
                ...
                catch (std::exception& e)
                {
                    ...
                    {
                        BinaryMessage raw =
                            read_rawmsg(files[i], Encoding::CREX);
                        unique_ptr<Bulletin> bulletin(
                            CrexBulletin::decode(raw.data));
                        bulletin->print(stderr);
                    }
                    catch (std::exception& e1)
                    {
        ...
        add_crex_method("test-synop0.crex", [](const impl::Messages& msgs) {
            ...
            IS(station, 837);
            IS(st_type, 1);
            wassert(actual(msg.get_datetime()) ==
                    Datetime(2004, 11, 30, 12, 0));
            IS(latitude, 48.22);
            IS(longitude, 9.92);
            IS(height_station, 550.0);
            UN(height_baro);
            IS(press, 94340.0);
        ...
        add_bufr_simplified_method(
            "obs0-1.22.bufr", [](const impl::Messages& msgs) {
                ...
                IS(latitude, 33.88);
                IS(longitude, -5.53);
                IS(height_station, 560.0);
                UN(height_baro);
                IS(press, 94190.0);
                IS(press_msl, 100540.0);
                IS(press_3h, -180.0);
                IS(press_tend, 8.0);
                IS(wind_dir, 80.0);
                IS(wind_speed, 4.0);
                IS(temp_2m, 289.2);
                IS(dewpoint_2m, 285.7);
                UN(humidity);
                IS(visibility, 8000.0);
                IS(pres_wtr, 2);
                IS(past_wtr1_6h, 6);
                IS(past_wtr2_6h, 2);
                IS(cloud_n, 100);
        ...
        add_bufr_simplified_method(
            "synop-cloudbelow.bufr", [](const impl::Messages& msgs) {
                ...
                IS(st_type, 1);
                wassert(actual(msg.get_datetime()) ==
                        Datetime(2009, 12, 3, 15, 0));
                IS(latitude, 50.07361);
                IS(longitude, 12.40333);
                IS(height_station, 483.0);
                IS(height_baro, 490.0);
                IS(press, 95090.0);
                IS(press_msl, 101060.0);
                IS(press_3h, -110.0);
                IS(press_tend, 6.0);
                IS(wind_dir, 0.0);
                IS(wind_speed, 1.0);
                IS(temp_2m, 273.05);
                IS(dewpoint_2m, 271.35);
                IS(humidity, 88.0);
                IS(visibility, 14000.0);
                IS(pres_wtr, 508);
                IS2(WR_VAR(0, 20, 4), Level(1), Trange(205, 0, 10800),
                    10); // past_wtr1
                IS2(WR_VAR(0, 20, 5), Level(1), Trange(205, 0, 10800),
                    10); // past_wtr2
                IS(cloud_n, 38);
                IS(cloud_nh, 0);
                IS(cloud_hh, 6000.0);
        ...
        add_bufr_method(
            "synop-cloudbelow.bufr", [](const impl::Messages& msgs) {
                ...
                IS(height_station, 483.0);
                IS(height_baro, 490.0);
                IS2(WR_VAR(0, 10, 4), Level(102, 490000), Trange::instant(),
                    95090.0); // press
                IS2(WR_VAR(0, 10, 51), Level(102, 490000), Trange::instant(),
                    101060.0); // press_msl
                IS2(WR_VAR(0, 10, 63), Level(102, 490000),
                    Trange(205, 0, 10800), 6.0); // press_tend
                IS2(WR_VAR(0, 10, 60), Level(102, 490000), Trange(4, 0, 10800),
                    -110.0); // press_3h
                IS2(WR_VAR(0, 11, 1), Level(103, 10000), Trange(200, 0, 600),
                    0.0); // wind_dir
                IS2(WR_VAR(0, 11, 2), Level(103, 10000), Trange(200, 0, 600),
                    1.0); // wind_speed
                IS2(WR_VAR(0, 12, 101), Level(103, 2050), Trange::instant(),
                    273.05); // temp_2m
                IS2(WR_VAR(0, 12, 103), Level(103, 2050), Trange::instant(),
                    271.35); // dewpoint_2m
                IS2(WR_VAR(0, 13, 3), Level(103, 2050), Trange::instant(),
                    88.0); // humidity
                IS2(WR_VAR(0, 20, 1), Level(103, 8000), Trange::instant(),
                    14000.0); // visibility
                IS(pres_wtr, 508);
                IS2(WR_VAR(0, 20, 4), Level(1), Trange(205, 0, 10800),
                    10); // past_wtr1
        ...
        add_bufr_simplified_method(
            "temp-2-255.bufr", [](const impl::Messages& msgs) {
                ...
                wassert(
                    actual(msg.find_context(Level(100, -1), Trange::instant()))
                        .isfalse());
            });
        ...
        add_bufr_simplified_method(
            "synop-longname.bufr", [](const impl::Messages& msgs) {
                ...
                // Check that the long station name has been correctly truncated
                // on import
                const Var* var = msg.get_st_name_var();
                wassert(actual(var).istrue());
                wassert(actual(string(var->enqc())) ==
                        "Budapest Pestszentlorinc-kulteru");
            });
        ...
        add_bufr_simplified_method(
            "temp-bad2.bufr", [](const impl::Messages& msgs) {
                wassert(actual(msgs.size()) == 1u);
                const impl::Message& msg = impl::Message::downcast(*msgs[0]);
                wassert(actual(msg.type) == MessageType::TEMP);
            });
        ...
        add_bufr_simplified_method(
            "temp-bad3.bufr", [](const impl::Messages& msgs) {
                wassert(actual(msgs.size()) == 1u);
                const impl::Message& msg = impl::Message::downcast(*msgs[0]);
                wassert(actual(msg.type) == MessageType::TEMP);
            });
        ...
        add_bufr_simplified_method(
            "temp-bad4.bufr", [](const impl::Messages& msgs) {
                wassert(actual(msgs.size()) == 1u);
                const impl::Message& msg = impl::Message::downcast(*msgs[0]);
                wassert(actual(msg.type) == MessageType::TEMP);
            });
        ...
        // ECWMF AMDAR
        add_bufr_simplified_method(
            "obs4-144.4.bufr", [](const impl::Messages& msgs) {
                wassert(actual(msgs.size()) == 1u);
                const impl::Message& msg = impl::Message::downcast(*msgs[0]);
        ...
        // ECWMF ACARS
        add_bufr_simplified_method(
            "obs4-145.4.bufr", [](const impl::Messages& msgs) {
                wassert(actual(msgs.size()) == 1u);
                const impl::Message& msg = impl::Message::downcast(*msgs[0]);
        ...
        // WMO ACARS
        add_bufr_simplified_method(
            "gts-acars1.bufr", [](const impl::Messages& msgs) {
                wassert(actual(msgs.size()) == 1u);
                const impl::Message& msg = impl::Message::downcast(*msgs[0]);
                wassert(actual(msg.type) == MessageType::ACARS);
                IS(ident, "EU5331");
        ...
        // WMO ACARS
        add_bufr_simplified_method(
            "gts-acars2.bufr", [](const impl::Messages& msgs) {
                wassert(actual(msgs.size()) == 1u);
                const impl::Message& msg = impl::Message::downcast(*msgs[0]);
                wassert(actual(msg.type) == MessageType::ACARS);
                IS(ident, "FJCYR4RA");
        ...
        add_bufr_simplified_method(
            "gts-acars-uk1.bufr", [](const impl::Messages& msgs) {
                wassert(actual(msgs.size()) == 1u);
                const impl::Message& msg = impl::Message::downcast(*msgs[0]);
                // This contains the same data as an AMDAR and has undefined
                // subtype and localsubtype, so it gets identified as an AMDAR
                wassert(actual(msg.type) == MessageType::AMDAR);
                IS(ident, "EU3375");
            });
        ...
        // WMO ACARS US
        add_bufr_simplified_method(
            "gts-acars-us1.bufr", [](const impl::Messages& msgs) {
                wassert(actual(msgs.size()) == 1u);
                const impl::Message& msg = impl::Message::downcast(*msgs[0]);
                wassert(actual(msg.type) == MessageType::ACARS);
                IS(ident, "FJCYR4RA");
        ...
        // BUFR that has a variable that goes out of range when converted to
        // local B table
        add_method("outofrange", [] {
...

            {
                ...
                impl::Messages msgs = read_msgs("bufr/interpreted-range.bufr",
                                                Encoding::BUFR, *opts);
                wassert(actual(msgs.size()) == 1u);
                const impl::Message& msg = impl::Message::downcast(*msgs[0]);
        ...
        add_bufr_simplified_method(
            "pilot-gts2.bufr", [](const impl::Messages& msgs) {
                wassert(actual(msgs.size()) == 1u);
                const impl::Message& msg = impl::Message::downcast(*msgs[0]);
        ...
        // WMO PILOT, with pressure levels
        add_bufr_simplified_method(
            "temp-tsig-2.bufr", [](const impl::Messages& msgs) {
        ...
        // WMO pilot pressure
        add_bufr_simplified_method(
            "pilot-gts3.bufr", [](const impl::Messages& msgs) {
                wassert(actual(msgs.size()) == 1u);
                const impl::Message& msg = impl::Message::downcast(*msgs[0]);
                wassert(actual(msg.type) == MessageType::PILOT);
            });
        ...
        add_bufr_simplified_method(
            "pilot-gts4.bufr", [](const impl::Messages& msgs) {
                wassert(actual(msgs.size()) == 1u);
                const impl::Message& msg = impl::Message::downcast(*msgs[0]);
                wassert(actual(msg.type) == MessageType::PILOT);
            });
        ...
        // Wind profiler
        add_bufr_simplified_method(
            "temp-windprof1.bufr", [](const impl::Messages& msgs) {
                wassert(actual(msgs.size()) == 1u);
                const impl::Message& msg = impl::Message::downcast(*msgs[0]);
                wassert(actual(msg.type) == MessageType::TEMP);
            });
        ...
        // Precise import
        add_bufr_method(
            "gts-synop-linate.bufr", [](const impl::Messages& msgs) {
                wassert(actual(msgs.size()) == 1u);
                const impl::Message& msg = impl::Message::downcast(*msgs[0]);
                const Var* v = msg.get(Level(103, 2000), Trange(3, 0, 43200),
                                       WR_VAR(0, 12, 101));
        ...
        add_bufr_simplified_method("test-soil1.bufr", [](const impl::Messages&
                                                             msgs) {
            wassert(actual(msgs.size()) == 1u);
            const impl::Message& msg = impl::Message::downcast(*msgs[0]);
            wassert(actual(msg.type) == MessageType::SYNOP);
        ...
        add_bufr_simplified_method(
            "truncated-unicode.bufr", [](const impl::Messages& msgs) {
                wassert(actual(msgs.size()) == 1u);
                const impl::Message& msg = impl::Message::downcast(*msgs[0]);
                wassert(actual(msg.type) == MessageType::GENERIC);
                IS2(WR_VAR(0, 1, 19), Level(), Trange(),
