...
struct FlightBase : public Template
{
    ...
    void add(Varcode code) const { add(code, code); }
    ...
    void to_subset(const Message& msg, wreport::Subset& subset) override
    {
        ...
        for (const auto& ctx : msg.data)
        {
            ...
            switch (ctx.level.ltype1)
            {
                case 100:
                    use = ctx.values.maybe_var(sc::press.code) != nullptr ||
                          ctx.values.maybe_var(sc::height_station.code) !=
                              nullptr;
                    break;
                case 102:
                    use = ctx.values.maybe_var(sc::height_station.code) !=
                          nullptr;
                    break;
            }
            if (use)
            {
                if (flight_ctx != 0)
                    error_consistency::throwf("contradicting height indication "
                                              "found (both %d and %d)",
                                              flight_ctx->level.ltype1,
                                              ctx.level.ltype1);
                flight_ctx = &ctx;
            }
        }
        ...
        if (flight_ctx == 0)
            throw error_notfound(
                "no airplane pressure or height found in flight message");
    }
};
...
struct Airep : public FlightBase
{
    ...
    void to_subset(const Message& msg, wreport::Subset& subset) override
    {
        ...
        /* 10 */ add(
            WR_VAR(0, 7, 2),
            WR_VAR(0, 7, 30)); /* HEIGHT OF STATION -> HEIGHT OR ALTITUDE */
        /* 11 */ add(WR_VAR(0, 12, 1),
                     WR_VAR(0, 12, 101)); /* TEMPERATURE/DRY-BULB TEMPERATURE */
...
struct AmdarWMO : public FlightBase
{
    ...
    void to_subset(const Message& msg, wreport::Subset& subset) override
    {
        ...
        if (const wreport::Var* v =
                flight_ctx->values.maybe_var(WR_VAR(0, 7, 30)))
            add(WR_VAR(0, 7, 10), v);
        else if (flight_ctx->level.ltype1 == 102)
            subset.store_variable_d(WR_VAR(0, 7, 10),
                                    (double)flight_ctx->level.l1 / 1000.0);
        else
        ...
        /* 25 */ subset.store_variable_i(
            WR_VAR(0, 31, 1), 0); // FIXME: no replicated section so far
        // 102000 replicate 2 descriptors (delayed 031001) times
        //   011075 MEAN TURBULENCE INTENSITY (EDDY DISSIPATION
        //   RATE)[M**(2/3)/S] 011076 PEAK TURBULENCE INTENSITY (EDDY
        //   DISSIPATION RATE)[M**(2/3)/S]
        add(WR_VAR(0, 11, 37));
        add(WR_VAR(0, 11, 39));
        add(WR_VAR(0, 11, 77));
        add(WR_VAR(0, 20, 42));
...
struct Acars : public FlightBase
{
    ...
    void to_subset(const Message& msg, wreport::Subset& subset) override
    {
        ...
        /* 24 */ add(WR_VAR(0, 12, 1),
                     WR_VAR(0, 12, 101)); /* TEMPERATURE/DRY-BULB TEMPERATURE */
        /* 25 */ add(WR_VAR(0, 12, 3),
                     WR_VAR(0, 12, 103)); /* DEW-POINT TEMPERATURE */
