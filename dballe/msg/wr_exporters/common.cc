...
void ExporterModule::add(Varcode code, const msg::Context* ctx,
...
void CommonSynopExporter::init(const Message& msg, wreport::Subset& subset)
{
    ...
    for (int i = 0; i < 4; ++i)
        c_cloud_data[i] = 0;
    for (unsigned i = 0; i < sizeof(c_cloud_group) / sizeof(c_cloud_group[0]);
         ++i)
        c_cloud_group[i] = 0;
    c_wind     = 0;
    c_gust1    = 0;
    ...
    c_past_wtr = 0;
    c_depth    = 0;
    for (unsigned i = 0; i < sizeof(c_swell_waves) / sizeof(c_swell_waves[0]);
         ++i)
        c_swell_waves[i] = 0;
    v_press        = 0;
    v_pressmsl     = 0;
...
void CommonSynopExporter::scan_context(const msg::Context& c)
{
    ...
    switch (c.level.ltype1)
    {
        case 1:
            switch (c.trange.pind)
            {
                case 1:
                    ...
                    break;
                case 2:
                    if (c.values.maybe_var(WR_VAR(0, 12, 101)))
                        c_tmax = &c;
                    break;
                case 3:
                    if (c.values.maybe_var(WR_VAR(0, 12, 101)))
                ...
                case 205:
                    if (const Var* v = c.values.maybe_var(sc::press_tend.code))
                        v_ptend = v;
                    if (c.values.maybe_var(WR_VAR(0, 20, 4)) ||
                        c.values.maybe_var(WR_VAR(0, 20, 5)))
                        c_past_wtr = &c;
                    break;
                case 254:
        ...
        case 102:
            ...
            break;
        case 103:
            if (c.values.maybe_var(WR_VAR(0, 11, 1)) ||
                c.values.maybe_var(WR_VAR(0, 11, 2)))
                if (!c_wind)
                    c_wind = &c;
            if (c.values.maybe_var(WR_VAR(0, 11, 41)) ||
                c.values.maybe_var(WR_VAR(0, 11, 43)))
            {
                if (!c_gust1)
                    c_gust1 = &c;
                else if (!c_gust2)
            ...
            switch (c.trange.pind)
            {
                ...
                case 3:
                    ...
                    break;
                case 254:
                    if (c.values.maybe_var(sc::temp_2m.code) ||
                        c.values.maybe_var(sc::dewpoint_2m.code) ||
                        c.values.maybe_var(sc::humidity.code))
                        c_thermo = &c;
                    break;
            }
            break;
        ...
        case 256:
            ...
            switch (c.level.ltype2)
            {
                ...
                case 259:
                    if (c.level.l2 >= 0 &&
                        c.level.l2 < (int)(sizeof(c_cloud_group) /
                                           sizeof(c_cloud_group[0])))
                        c_cloud_group[c.level.l2] = &c;
                    break;
                case MISSING_INT: c_cloud_cover = &c; break;
            }
            break;
        case 264:
            // Swell wave groups
            switch (c.level.ltype2)
            {
                case 261:
                    if (c.level.l2 >= 1 &&
                        c.level.l2 <= (int)(sizeof(c_swell_waves) /
                                            sizeof(c_swell_waves[0])))
...
void CommonSynopExporter::add_D02052()
{
    ...
    else
    {
        ...
        const Var* var_d = c_thermo->values.maybe_var(sc::dewpoint_2m.code);
        const Var* var_h = c_thermo->values.maybe_var(sc::humidity.code);
        add_marine_sensor_height(*c_thermo, var_t   ? var_t
                                            : var_w ? var_w
                                            : var_d ? var_d
                                                    : var_h);
        add(WR_VAR(0, 12, 101), var_t);
...
void CommonSynopExporter::add_D02041()
{
    if (c_tmax || c_tmin)
    {
        ...
        add_sensor_height(*c_first,
                          c_first->values.maybe_var(WR_VAR(0, 12, 101)));
    }
    else
        subset->store_variable_undef(WR_VAR(0, 7, 32));
...
void CommonSynopExporter::add_D02058()
{
    ...
    {
        const msg::Context* c_first = c_tmax ? c_tmax : c_tmin;
        add_marine_sensor_height(*c_first,
                                 c_first->values.maybe_var(WR_VAR(0, 12, 101)));
    }
    else
    {
...
void CommonSynopExporter::add_D02042()
{
    if (c_wind || c_gust1 || c_gust2)
    {
        ...
        const msg::Context* c_first = c_wind    ? c_wind
                                      : c_gust1 ? c_gust1
                                                : c_gust2;
        const Var* sample_var = c_first->values.maybe_var(WR_VAR(0, 11, 1));
        if (!sample_var)
            sample_var = c_first->values.maybe_var(WR_VAR(0, 11, 2));
        if (!sample_var)
            sample_var = c_first->values.maybe_var(WR_VAR(0, 11, 41));
        if (!sample_var)
            sample_var = c_first->values.maybe_var(WR_VAR(0, 11, 43));
...
void CommonSynopExporter::add_sensor_height(const msg::Context& c,
                                            const Var* sample_var)
{
    ...
    else if (c.level.ltype1 == 103)
    {
        ...
        if (c.level.l1 == MISSING_INT)
            ...
            subset->store_variable_d(WR_VAR(0, 7, 32),
                                     double(c.level.l1) / 1000.0);
    }
    else
        error_consistency::throwf(
            "Cannot add sensor height from unsupported level type %d",
            c.level.ltype1);
}

void CommonSynopExporter::add_marine_sensor_height(const msg::Context& c,
                                                   const Var* sample_var)
{
...
void CommonSynopExporter::add_D02038()
{
    ...
    if (c_past_wtr)
    {
        ...
        if (!v)
            v = c_past_wtr->values.maybe_var(WR_VAR(0, 20, 5));
        add_time_period(WR_VAR(0, 4, 24), *c_past_wtr, v,
                        hour % 6 == 0 ? tr_std_past_wtr6 : tr_std_past_wtr3);
    }
    else
        subset->store_variable_undef(WR_VAR(0, 4, 24));
    add(WR_VAR(0, 20, 4), c_past_wtr);
...
void CommonSynopExporter::add_time_period(Varcode code, const msg::Context& c,
                                          const Var* sample_var,
                                          const Trange& tr_std)
{
    int p2;
...
void CommonSynopExporter::add_D02056()
{
    ...
    if (c_depth)
    {
        if (c_depth->level.ltype1 == 160)
            subset->store_variable_d(WR_VAR(0, 7, 63),
                                     (double)c_depth->level.l1 / 1000.0);
        else
...
void CommonSynopExporter::add_D02024()
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        for (; src_idx < sizeof(c_swell_waves) / sizeof(c_swell_waves[0]);
             ++src_idx)
