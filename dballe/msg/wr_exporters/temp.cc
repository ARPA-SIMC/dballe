...
struct TempBase : public Template
{
    ...
    int add_sounding_levels()
    {
        ...
        // Iterate backwards as we need to add levels in decreasing pressure
        // order
        for (auto i = msg->data.rbegin(); i != msg->data.rend(); ++i)
        {
            ...
            /* Add vertical sounding significance */
            {
                ...
                {
                    Var nvar(subset->tables->btable->query(WR_VAR(0, 8, 1)),
                             vssval);
                    nvar.setattrs(*vss);
...
struct TempWMO : public TempBase
{
        ...
        switch (c.level.ltype1)
        {
            ...
            case 102:
                ...
                if (const Var* var = c.values.maybe_var(WR_VAR(0, 10, 8)))
                    ...
                    subset->store_variable_d(WR_VAR(0, 10, 9),
                                             (double)c.level.l1);
                break;
            case 103:
                add(WR_VAR(0, 7, 4), &c, WR_VAR(0, 10, 4));
    ...
    void to_subset(const Message& msg, wreport::Subset& subset) override
    {
        ...
        add(WR_VAR(0, 8, 2), WR_VAR(0, 8, 2), Level::cloud(258, 0),
            Trange::instant());
        ...
        for (auto i = msg.data.rbegin(); i != msg.data.rend(); ++i)
        {
            ...
            if (!(c.values.maybe_var(WR_VAR(0, 10, 8)) ||
        ...
        for (auto i = msg.data.rbegin(); i != msg.data.rend(); ++i)
        {
            ...
            if (c.level.ltype1 != 100)
                continue;
            // Skip levels not for us
            if (!(c.values.maybe_var(WR_VAR(0, 11, 61)) ||
                  c.values.maybe_var(WR_VAR(0, 11, 62))))
                continue;
            if (do_D03051(c))
                ++group_count;
...
struct TempEcmwfLand : public TempBase
{
    ...
    void to_subset(const Message& msg, wreport::Subset& subset) override
    {
        ...
        /* 13 */ add(WR_VAR(0, 8, 2), WR_VAR(0, 8, 2), Level::cloud(258, 0),
                     Trange::instant());
        ...
        if (!is_crex)
        {
            int count = subset.append_dpb(WR_VAR(2, 22, 0), subset.size(),
                                          WR_VAR(0, 33, 7));
...
struct TempEcmwfShip : public TempBase
{
    ...
    void setupBulletin(wreport::Bulletin& bulletin) override
    {
        ...
        /* FIXME: some expansions are not in CREX D tables yet, so we need to
         * use the expanded version here.  The code needs to be changed when
         * newer CREX D tables are available */
    ...
    void to_subset(const Message& msg, wreport::Subset& subset) override
    {
        ...
        /* 14 */ add(WR_VAR(0, 8, 2), WR_VAR(0, 8, 2), Level::cloud(258, 0),
                     Trange::instant());
...
struct PilotWMO : public TempBase
{
    ...
    void setupBulletin(wreport::Bulletin& bulletin) override
    {
        ...
        if (pressure_levs)
            bulletin.datadesc.push_back(
                WR_VAR(3, 9, 50)); // For pressure levels
        else
    ...
    void to_subset(const Message& msg, wreport::Subset& subset) override
    {
        ...
        for (auto i = msg.data.rbegin(); i != msg.data.rend(); ++i)
        {
            ...
            if (vss == NULL)
                continue;
            if (pressure_levs && c.level.ltype1 != 100)
                continue;
            if (!pressure_levs && c.level.ltype1 != 102)
                continue;
...
struct PilotEcmwf : public TempBase
{
    ...
    void to_subset(const Message& msg, wreport::Subset& subset) override
    {
        ...
        for (auto i = msg.data.rbegin(); i != msg.data.rend(); ++i)
        {
            ...
            {
                Var nvar(subset.tables->btable->query(WR_VAR(0, 8, 1)),
                         (int)convert_BUFR08042_to_BUFR08001(vss->enqi()));
                nvar.setattrs(*vss);
                subset.store_variable(WR_VAR(0, 8, 1), nvar);
            }
            ...
            if (const Var* var = c.values.maybe_var(WR_VAR(0, 10, 3)))
                ...
                subset.store_variable_d(WR_VAR(0, 10, 3),
                                        (double)c.level.l1 * 9.80665);
            else
...
{
    r.register_factory(
        2, "temp", "Temp (autodetect)",
        [](const dballe::ExporterOptions& opts, const Messages& msgs) {
            // Get the type of equipment used
            if (const wreport::Var* var =
                    msgs[0]->get(Level(1), Trange::instant(), WR_VAR(0, 2, 3)))
                // Is it a Radar?
                if (var->enq(0) == 3)
            ...
            // ECMWF temps use normal replication which cannot do more than 256
            // levels
            ...
            {
                return wr::TemplateRegistry::get("temp-ecmwf")
                    .factory(opts, msgs);
            }
        });
    r.register_factory(
        2, "temp-ship", "Temp ship (autodetect)",
        [](const dballe::ExporterOptions& opts, const Messages& msgs) {
            return unique_ptr<Template>(new TempEcmwfShip(opts, msgs));
        });
    r.register_factory(
        2, TEMP_WMO_NAME, TEMP_WMO_DESC,
        [](const dballe::ExporterOptions& opts, const Messages& msgs) {
            return unique_ptr<Template>(new TempWMO(opts, msgs));
        });
    r.register_factory(
        2, "temp-ecmwf", "Temp ECMWF (autodetect)",
        [](const dballe::ExporterOptions& opts, const Messages& msgs) {
            if (msgs.empty() ||
                Message::downcast(msgs[0])->type != MessageType::TEMP_SHIP)
                return unique_ptr<Template>(new TempEcmwfLand(opts, msgs));
            else
                return unique_ptr<Template>(new TempEcmwfShip(opts, msgs));
        });
    r.register_factory(
        2, TEMP_ECMWF_LAND_NAME, TEMP_ECMWF_LAND_DESC,
        [](const dballe::ExporterOptions& opts, const Messages& msgs) {
            return unique_ptr<Template>(new TempEcmwfLand(opts, msgs));
        });
    r.register_factory(
        2, TEMP_ECMWF_SHIP_NAME, TEMP_ECMWF_SHIP_DESC,
        [](const dballe::ExporterOptions& opts, const Messages& msgs) {
            return unique_ptr<Template>(new TempEcmwfShip(opts, msgs));
        });
    r.register_factory(
        6, TEMP_RADAR_NAME, TEMP_RADAR_DESC,
        [](const dballe::ExporterOptions& opts, const Messages& msgs) {
            return unique_ptr<Template>(new TempRadar(opts, msgs));
        });
    r.register_factory(
        2, PILOT_WMO_NAME, PILOT_WMO_DESC,
        [](const dballe::ExporterOptions& opts, const Messages& msgs) {
            return unique_ptr<Template>(new PilotWMO(opts, msgs));
        });
    r.register_factory(
        2, PILOT_ECMWF_NAME, PILOT_ECMWF_DESC,
        [](const dballe::ExporterOptions& opts, const Messages& msgs) {
            return unique_ptr<Template>(new PilotEcmwf(opts, msgs));
        });
    r.register_factory(
        2, "pilot", "pilot (autodetect)",
        [](const dballe::ExporterOptions& opts, const Messages& msgs) {
            auto msg       = Message::downcast(msgs[0]);
            const Var* var = msg->get_sonde_tracking_var();
            // Try with another one in case the first was just unset
            if (!var)
                var = msg->get_meas_equip_type_var();
            if (var)
                return unique_ptr<Template>(new PilotWMO(opts, msgs));
            else
                return unique_ptr<Template>(new PilotEcmwf(opts, msgs));
