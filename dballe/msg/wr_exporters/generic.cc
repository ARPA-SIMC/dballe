...
struct Generic : public Template
{
    ...
    void add_var_and_attrs(const Var& var)
    {
        ...
        for (const Var* attr = var.next_attr(); attr != NULL;
             attr            = attr->next_attr())
        {
            if (WR_VAR_X(attr->code()) != 33)
                ...
                if (WR_VAR_X(attr->code()) != 33)
                    continue;
            // error_consistency::throwf("attempt to encode attribute B%02d%03d
            // which is not B33YYY",
    ...
    void setupBulletin(wreport::Bulletin& bulletin) override
    {
        ...
        {
            // TODO: change to BUFR-like info when we can encode BUFR table info
            // in Crex
    ...
    void to_subset(const Message& msg, wreport::Subset& subset) override
    {
        ...
        else if (msg.type != MessageType::GENERIC) // It is generic by default,
                                                   // no need to repeat it
            subset.store_variable_c(WR_VAR(0, 1, 194),
                                    Message::repmemo_from_type(msg.type));
        else
        ...
        for (const auto& ctx : msg.data)
        {
            for (const auto& val : ctx.values)
            {
                const Var& var = *val;
                if (!var.isset())
                    continue; // Don't add undef vars
                if (&var == repmemo)
                    continue; // Don't add rep_memo again
                ...
                if (lev.ltype1 != ctx.level.ltype1)
                {
                    if (ctx.level.ltype1 == MISSING_INT)
                        ...
                        subset.store_variable_i(WR_VAR(0, 7, 192),
                                                ctx.level.ltype1);
                ...
                if (lev.l1 != ctx.level.l1)
                {
                    if (ctx.level.l1 == MISSING_INT)
                        ...
                        subset.store_variable_i(WR_VAR(0, 7, 193),
                                                ctx.level.l1);
                    lev.l1 = ctx.level.l1;
                }
                if (lev.ltype2 != ctx.level.ltype2)
                {
                    if (ctx.level.ltype2 == MISSING_INT)
                        ...
                        subset.store_variable_i(WR_VAR(0, 7, 195),
                                                ctx.level.ltype2);
                ...
                if (lev.l2 != ctx.level.l2)
                {
                    if (ctx.level.l2 == MISSING_INT)
                        ...
                        subset.store_variable_i(WR_VAR(0, 7, 194),
                                                ctx.level.l2);
                    lev.l2 = ctx.level.l2;
                }
                if (tr.pind != ctx.trange.pind)
                {
                    if (ctx.trange.pind == MISSING_INT)
                        ...
                        subset.store_variable_i(WR_VAR(0, 4, 192),
                                                ctx.trange.pind);
                ...
                if (tr.p1 != ctx.trange.p1)
                {
                    if (ctx.trange.p1 == MISSING_INT)
                        ...
                        subset.store_variable_i(WR_VAR(0, 4, 193),
                                                ctx.trange.p1);
                    tr.p1 = ctx.trange.p1;
                }
                if (tr.p2 != ctx.trange.p2)
                {
                    if (ctx.trange.p2 == MISSING_INT)
                        ...
                        subset.store_variable_i(WR_VAR(0, 4, 194),
                                                ctx.trange.p2);
                    tr.p2 = ctx.trange.p2;
                }
