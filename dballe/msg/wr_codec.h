...
public:
     ...
     */
    std::vector<std::shared_ptr<dballe::Message>>
    from_bulletin(const wreport::Bulletin& msg) const override;
...
{
public:
    BufrImporter(const dballe::ImporterOptions& opts =
                     dballe::ImporterOptions::defaults);
    virtual ~BufrImporter();

    Encoding encoding() const override { return Encoding::BUFR; }

    bool foreach_decoded(const BinaryMessage& msg,
                         std::function<bool(std::shared_ptr<dballe::Message>)>
                             dest) const override;
};
...
{
public:
    CrexImporter(const dballe::ImporterOptions& opts =
                     dballe::ImporterOptions::defaults);
...
public:
    ...
    to_bulletin(const std::vector<std::shared_ptr<dballe::Message>>& msgs)
        const override;
...
{
public:
    BufrExporter(const dballe::ExporterOptions& opts =
                     dballe::ExporterOptions::defaults);
...
{
public:
    CrexExporter(const dballe::ExporterOptions& opts =
                     dballe::ExporterOptions::defaults);
...
protected:
    ...
    void add(wreport::Varcode code, const Values& values,
             const Shortcut& shortcut) const;
    void add(wreport::Varcode code, const Shortcut& shortcut) const;
    void add(wreport::Varcode code, wreport::Varcode srccode,
             const Level& level, const Trange& trange) const;
...
{
    typedef std::function<std::unique_ptr<Template>(
    ...
    TemplateFactory(unsigned data_category, std::string name,
                    std::string description, factory_func factory)
        : data_category(data_category), name(name), description(description),
          factory(factory)
    {
    }
};
...
struct TemplateRegistry : public std::map<std::string, TemplateFactory>
{
    ...
    void register_factory(unsigned data_category, const std::string& name,
