...
dba_err bufrex_copy_to_sat(dba_msg msg, bufrex_msg raw, bufrex_subset sset)
{
	...
	-- Ignored
			...
			case DBA_VAR(0, 5, 41): scanline = var; break;
			// 005043 FIELD OF VIEW NUMBER(NUMERIC): 0
			case DBA_VAR(0, 5, 43): fov = var; break;
			// 025070 MAJOR FRAME COUNT(NUMERIC): (undef)
			case DBA_VAR(0, 25, 70): framecount = var; break;
			// -> Attrs
	...
	-- Ignored
			...
			case DBA_VAR(0, 2, 150): DBA_RUN_OR_RETURN(dba_var_enqi(var, &l1)); break;
			// -> level (8, x, 0)

			// 025076 LOG-10 OF (TEMP-RAD CENTRAL WAVENUMBER) FOR ATOVS(LOGM-1): 4.825342
			case DBA_VAR(0, 25, 76): wavenum = var; break;
			// -> attr
#if 0																			  
	-- Ignored
			...
			case DBA_VAR(0, 33, 32): chqf = var; break;
			// -> attr
