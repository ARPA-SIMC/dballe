...
struct CursorStation : public impl::CursorStation
{
    ...
    bool has_value() const override { return !at_start; }
    ...
    void discard() override { at_start = false; }

    void enq(Enq& enq) const override;

    DBStation get_station() const override { return station; }

    DBValues get_values() const override { return DBValues(station_values); }
...
struct CursorDataRow
{
    ...
    CursorDataRow(Values::const_iterator var) : var(var) {}

    CursorDataRow(const Level& level, const Trange& trange,
                  Values::const_iterator var)
...
struct CursorData : public impl::CursorData
{
    ...
    CursorData(const impl::Message& msg, bool merged = false)
    {
        ...
        for (const auto& ctx : msg.data)
            for (Values::const_iterator cur = ctx.values.begin();
                 cur != ctx.values.end(); ++cur)
                rows.emplace_back(ctx.level, ctx.trange, cur);

        if (merged)
            for (Values::const_iterator cur = msg.station_data.begin();
                 cur != msg.station_data.end(); ++cur)
                if (WR_VAR_X((*cur)->code()) < 4 ||
                    WR_VAR_X((*cur)->code()) > 6)
                    rows.emplace_back(cur);
    }
    ~CursorData();

    bool has_value() const override { return !at_start && cur != rows.end(); }
