...
public:
    SynopImporter(const dballe::ImporterOptions& opts) : SynopBaseImporter(opts)
    {
    }
...
void SynopImporter::import_var(const Var& var)
{
    switch (var.code())
    {
        ...
        case WR_VAR(0, 5, 21):
            set(var, WR_VAR(0, 5, 21), Level::cloud(262, 0), Trange::instant());
            break;
        case WR_VAR(0, 7, 21):
            set(var, WR_VAR(0, 7, 21), Level::cloud(262, 0), Trange::instant());
            break;
        ...
        case WR_VAR(0, 12, 113):
            set(var, WR_VAR(0, 12, 121), Level(1), Trange(3, 0, 43200));
            break;
        ...
        case WR_VAR(0, 14, 30):
            if (trange.time_period == MISSING_INT)
                ...
                set(var, var.code(), Level(1),
                    Trange(1, 0, abs(trange.time_period)));
            break;

        // Temperature change
        case WR_VAR(0, 12, 49):
            set(var, WR_VAR(0, 12, 49), Level(1),
                Trange(4, -abs(trange.time_period_offset),
...
Importer::createSynop(const dballe::ImporterOptions& opts)
{
