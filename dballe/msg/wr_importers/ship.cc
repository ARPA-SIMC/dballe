...
public:
    ShipImporter(const dballe::ImporterOptions& opts) : SynopBaseImporter(opts)
    {
    }
                    ...
                    case 0:  {
                        ...
                        if (bulletin.subsets.empty())
                            throw error_consistency(
                                "trying to import a SYNOP message with no data "
                                "subset");
                        const Subset& subset = bulletin.subsets[0];
                        if (subset.size() > 1 &&
                            subset[0].code() == WR_VAR(0, 1, 5))
...
void ShipImporter::import_var(const Var& var)
{
    switch (var.code())
    {
        ...
        case WR_VAR(0, 20, 37):
        case WR_VAR(0, 20, 38):
            msg->set(Level(1), Trange::instant(), var.code(), var);
            break;
        ...
        case WR_VAR(0, 22, 43):
            if (level.sea_depth == LevelContext::missing)
                ...
                set(var, WR_VAR(0, 22, 43), Level(161, level.sea_depth * 1000),
                    Trange::instant());
            break;
        ...
        case WR_VAR(0, 22, 23): // Height of swell waves
            set(var, var.code(),
                Level(264, MISSING_INT, 261, level.swell_wave_group),
                Trange::instant());
            break;
...
Importer::createShip(const dballe::ImporterOptions& opts)
{
