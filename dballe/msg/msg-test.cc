...
void init(Messages& m) { m.emplace_back(make_shared<impl::Message>()); }

// Ensure that the context vector inside the message is in strict ascending
// order
...
void Tests::register_tests()
{
    ...
    add_method("ordering", []() {
        ...
        msg.set(lev2, tr1, move(av1));
        wassert(actual(msg.data.size()) == 1);
        msg.set(lev2, tr1, move(av2));
        wassert(actual(msg.data.size()) == 1);
        msg.set(lev1, tr1, move(av3));
        wassert(actual(msg.data.size()) == 2);
        msg.set(lev1, tr2, move(av4));
        wassert(actual(msg.data.size()) == 3);
        ...
        wassert(actual(msg.get(Level(0, 0, 0, 0), tr1, WR_VAR(0, 1, 1))) ==
                (Var*)0);
        wassert(actual(msg.get(Level(3, 3, 3, 3), tr1, WR_VAR(0, 1, 1))) ==
                (Var*)0);
        wassert(actual(msg.get(lev1, Trange(3, 3, 3), WR_VAR(0, 1, 1))) ==
                (Var*)0);
    });
    ...
    add_method("compose", []() {
        ...
        // ...
        auto var = newvar(WR_VAR(0, 12, 101), 273.0);
        var->seta(newvar(WR_VAR(0, 33, 7), 75));
    ...
    add_method("repmemo", []() {
        ...
        wassert(actual(impl::Message::type_from_repmemo(
                    impl::Message::repmemo_from_type(MessageType::SYNOP))) ==
                MessageType::SYNOP);
        wassert(actual(impl::Message::type_from_repmemo(
                    impl::Message::repmemo_from_type(MessageType::METAR))) ==
                MessageType::METAR);
        wassert(actual(impl::Message::type_from_repmemo(
                    impl::Message::repmemo_from_type(MessageType::SHIP))) ==
                MessageType::SHIP);
        wassert(actual(impl::Message::type_from_repmemo(
                    impl::Message::repmemo_from_type(MessageType::BUOY))) ==
                MessageType::BUOY);
        wassert(actual(impl::Message::type_from_repmemo(
                    impl::Message::repmemo_from_type(MessageType::AIREP))) ==
                MessageType::AIREP);
        wassert(actual(impl::Message::type_from_repmemo(
                    impl::Message::repmemo_from_type(MessageType::AMDAR))) ==
                MessageType::AMDAR);
        wassert(actual(impl::Message::type_from_repmemo(
                    impl::Message::repmemo_from_type(MessageType::ACARS))) ==
                MessageType::ACARS);
        wassert(actual(impl::Message::type_from_repmemo(
                    impl::Message::repmemo_from_type(MessageType::PILOT))) ==
                MessageType::PILOT);
        wassert(actual(impl::Message::type_from_repmemo(
                    impl::Message::repmemo_from_type(MessageType::TEMP))) ==
                MessageType::TEMP);
        wassert(actual(impl::Message::type_from_repmemo(
                    impl::Message::repmemo_from_type(
                        MessageType::TEMP_SHIP))) == MessageType::TEMP_SHIP);
        wassert(actual(impl::Message::type_from_repmemo(
                    impl::Message::repmemo_from_type(MessageType::SAT))) ==
                MessageType::SAT);
        wassert(actual(impl::Message::type_from_repmemo(
                    impl::Message::repmemo_from_type(
                        MessageType::POLLUTION))) == MessageType::POLLUTION);
        wassert(actual(impl::Message::type_from_repmemo(
                    impl::Message::repmemo_from_type(MessageType::GENERIC))) ==
                MessageType::GENERIC);

        wassert(actual(impl::Message::type_from_repmemo("synop")) ==
                MessageType::SYNOP);
        wassert(actual(impl::Message::type_from_repmemo("SYNOP")) ==
                MessageType::SYNOP); // Case insensitive
        wassert(actual(impl::Message::type_from_repmemo("metar")) ==
                MessageType::METAR);
        wassert(actual(impl::Message::type_from_repmemo("ship")) ==
                MessageType::SHIP);
        wassert(actual(impl::Message::type_from_repmemo("buoy")) ==
                MessageType::BUOY);
        wassert(actual(impl::Message::type_from_repmemo("airep")) ==
                MessageType::AIREP);
        wassert(actual(impl::Message::type_from_repmemo("amdar")) ==
                MessageType::AMDAR);
        wassert(actual(impl::Message::type_from_repmemo("acars")) ==
                MessageType::ACARS);
        wassert(actual(impl::Message::type_from_repmemo("pilot")) ==
                MessageType::PILOT);
        wassert(actual(impl::Message::type_from_repmemo("temp")) ==
                MessageType::TEMP);
        wassert(actual(impl::Message::type_from_repmemo("tempship")) ==
                MessageType::TEMP_SHIP);
        wassert(actual(impl::Message::type_from_repmemo("satellite")) ==
                MessageType::SAT);
        wassert(actual(impl::Message::type_from_repmemo("pollution")) ==
                MessageType::POLLUTION);
        wassert(actual(impl::Message::type_from_repmemo("generic")) ==
                MessageType::GENERIC);
        wassert(actual(impl::Message::type_from_repmemo("antani")) ==
                MessageType::GENERIC);
        wassert(actual(impl::Message::type_from_repmemo("")) ==
                MessageType::GENERIC);
        wassert(actual(impl::Message::type_from_repmemo(NULL)) ==
                MessageType::GENERIC);
    });
    ...
    add_method("msg_match_stationid", []() {
        ...
        wassert(actual_matcher_result(m->match(MatchedMsg(matched))) ==
                matcher::MATCH_NO);
    ...
    add_method("msgs_match_stationid", []() {
        ...
        impl::Messages matched;
        init(matched);
        wassert(actual_matcher_result(m->match(MatchedMessages(matched))) ==
                matcher::MATCH_NO);

        impl::Message::downcast(matched[0])
            ->station_data.set(var(WR_VAR(0, 1, 192), 2));
        wassert(actual_matcher_result(m->match(MatchedMessages(matched))) ==
                matcher::MATCH_NO);

        impl::Message::downcast(matched[0])
            ->station_data.set(var(WR_VAR(0, 1, 192), 1));
        wassert(actual_matcher_result(m->match(MatchedMessages(matched))) ==
                matcher::MATCH_YES);
    });
    ...
    add_method("msgs_csv", []() {
        // Test CSV encoding/decoding
        impl::Messages msgs =
            read_msgs("bufr/synop-evapo.bufr", Encoding::BUFR);
    ...
    add_method("iterate_no_data", [] {
        // Check that a message that only contains station variables does get
        // imported
        impl::Messages msgs =
