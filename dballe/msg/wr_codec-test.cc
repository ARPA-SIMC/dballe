...
void Tests::register_tests()
{
    ...
    add_method("issue239", []() {
        ...
        msg->obtain_context(Level(1), Trange(254, 0, 0))
            .values.set(WR_VAR(0, 13, 211), 12.123);
        ...
        const wreport::Var* var =
            imported[0]->get(Level(1), Trange(254, 0, 0), WR_VAR(0, 13, 211));
    ...
    add_method("domain_throw", []() {
        auto file =
            File::create(Encoding::BUFR,
                         tests::datafile("bufr/interpreted-range.bufr"), "r");
        ...
        file->foreach ([&](const BinaryMessage& bmsg) {
            wassert_throws(
                ...
                importer->foreach_decoded(
                    bmsg, [&](std::shared_ptr<Message> dest) noexcept {
                        return true;
                    }));
            return true;
        });
    ...
    add_method("domain_unset", []() {
        ...
        wassert_true(file->foreach ([&](const BinaryMessage& bmsg) {
            return importer->foreach_decoded(
                bmsg, [&](std::shared_ptr<Message> dest) {
                    // Would throw: wreport::error_domain: Value 329.2 is
                    // outside the range [0,327.66] for B22043 (SEA/WATER
                    // TEMPERATURE)
                    const wreport::Var* val = dest->get(
                        Level(1), Trange::instant(), WR_VAR(0, 22, 43));
                    wassert_true(val);
