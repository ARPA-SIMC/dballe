		...
		TRAIL(parameter))
{
	...
	try {
		...
		{
			TRACEMISSING("int");
            h.trace.log_unset(parm);
			h.api->unset(parm);
		}
		else
        {
            h.trace.log_set(parm, *value);
		...
		INTEGER(p2))
{
	...
	try {
		HSimple& h = hsimp.get(*handle);
        h.trace.log_settimerange(fromfortran(*ptype), fromfortran(*p1), fromfortran(*p2));
		h.api->settimerange(
