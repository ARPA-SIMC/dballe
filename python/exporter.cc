...
struct to_binary : MethKwargs<to_binary, dpy_Exporter>
{
    ...
    constexpr static const char* signature =
        "contents: Union[dballe.Message, Sequence[dballe.Message], "
        "Iterable[dballe.Message]]";
...
Encode a :class:`dballe.Message` or a sequence of :class:`dballe.Message` into a bytes object.
    ...
    [[noreturn]] static void throw_typeerror()
    {
        PyErr_SetString(
            PyExc_ValueError,
            "to_binary requires a dballe.Message or a non-empty sequence of "
            "dballe.Message objects, or an iterable of dballe.Message objects");
    ...
    static PyObject* run(Impl* self, PyObject* args, PyObject* kw)
    {
        ...
        if (!PyArg_ParseTupleAndKeywords(args, kw, "O",
                                         const_cast<char**>(kwlist), &contents))
            return nullptr;
        try
        {
            ...
            else if (PySequence_Check(contents))
            {
                ...
                {
                    pyo_unique_ptr o(
                        throw_ifnull(PySequence_ITEM(contents, i)));
            ...
            {
                // Iterate iterator
                pyo_unique_ptr iterator =
                    throw_ifnull(PyObject_GetIter(contents));
...
the contents of a :class:`dballe.Message`.
    ...
    {
        static const char* kwlist[] = {"encoding",  "template_name", "centre",
                                       "subcentre", "application",   nullptr};
        ...
        if (!PyArg_ParseTupleAndKeywords(
                args, kw, "s|siii", const_cast<char**>(kwlist), &encoding,
                &template_name, &centre, &subcentre, &application))
            return -1;
        ...
        try
        {
            ...
            if (centre != -1)
                opts.centre = centre;
            if (subcentre != -1)
                opts.subcentre = subcentre;
            if (application != -1)
                opts.application = application;
            self->exporter =
                Exporter::create(File::parse_encoding(encoding), opts)
                    .release();
        }
