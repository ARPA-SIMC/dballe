...
typedef struct
{
    PyObject_HEAD dballe::Level val;
} dpy_Level;

extern PyTypeObject* dpy_Level_Type;

#define dpy_Level_Check(ob)                                                    \
    (ob == Py_None || PyTuple_Check(ob) || Py_TYPE(ob) == dpy_Level_Type ||    \
     PyType_IsSubtype(Py_TYPE(ob), dpy_Level_Type))

typedef struct
{
    PyObject_HEAD dballe::Trange val;
} dpy_Trange;

extern PyTypeObject* dpy_Trange_Type;

#define dpy_Trange_Check(ob)                                                   \
    (ob == Py_None || PyTuple_Check(ob) || Py_TYPE(ob) == dpy_Trange_Type ||   \
     PyType_IsSubtype(Py_TYPE(ob), dpy_Trange_Type))

typedef struct
{
    PyObject_HEAD dballe::Station val;
} dpy_Station;

extern PyTypeObject* dpy_Station_Type;

#define dpy_Station_Check(ob)                                                  \
    (Py_TYPE(ob) == dpy_Station_Type ||                                        \
     PyType_IsSubtype(Py_TYPE(ob), dpy_Station_Type))

typedef struct
{
    PyObject_HEAD dballe::DBStation val;
} dpy_DBStation;

extern PyTypeObject* dpy_DBStation_Type;

#define dpy_DBStation_Check(ob)                                                \
    (Py_TYPE(ob) == dpy_DBStation_Type ||                                      \
     PyType_IsSubtype(Py_TYPE(ob), dpy_DBStation_Type))
}
...
inline PyObject* to_python(const Datetime& dt)
{
    return datetime_to_python(dt);
}

/// Convert a python datetime object to a Datetime
Datetime datetime_from_python(PyObject* dt);
template <> inline Datetime from_python<Datetime>(PyObject* o)
{
    return datetime_from_python(o);
}

/// Convert a sequence of two python datetime objects to a DatetimeRange
DatetimeRange datetimerange_from_python(PyObject* dt);
template <> inline DatetimeRange from_python<DatetimeRange>(PyObject* o)
{
    return datetimerange_from_python(o);
}

// inline PyObject* to_python(const DatetimeRange& dtr) { return
// datetimerange_to_python(dtr); }
...
template <> inline Ident from_python<Ident>(PyObject* o)
{
    return ident_from_python(o);
}
...
template <> inline Level from_python<Level>(PyObject* o)
{
    return level_from_python(o);
}
...
template <> inline Trange from_python<Trange>(PyObject* o)
{
    return trange_from_python(o);
}
...
template <> inline Station from_python<Station>(PyObject* o)
{
    return station_from_python(o);
}

/// Convert a Station to a python Station structseq
PyObject* dbstation_to_python(const DBStation& s);
inline PyObject* to_python(const DBStation& s)
{
    return dbstation_to_python(s);
}

/// Convert a structseq to a Station
DBStation dbstation_from_python(PyObject* o);
template <> inline DBStation from_python<DBStation>(PyObject* o)
{
    return dbstation_from_python(o);
}

/// Convert a varcode to a Python string
PyObject* varcode_to_python(wreport::Varcode code);
inline PyObject* to_python(const wreport::Varcode& s)
{
    return varcode_to_python(s);
}

/// Convert a Python object to a Varcode
wreport::Varcode varcode_from_python(PyObject* o);
template <> inline wreport::Varcode from_python<wreport::Varcode>(PyObject* o)
{
    return varcode_from_python(o);
}
...
set_values_from_python(Values& values, wreport::Varcode code, PyObject* val);
extern template void
set_values_from_python(DBValues& values, wreport::Varcode code, PyObject* val);
...
void query_setpy(core::Query& query, const char* key, unsigned len,
                 PyObject* val);
