...
\item When writing, you can use the {\tt query} input parameter to {\tt
      ...
      of the names defined in \ref{tbl-templates}, for example: ``{\tt message
      generic}''.  If you do not specify a template, an appropriate template
...
\chapter{Message template aliases}

These are the names that can be used to specify a BUFR or CREX message template:

\label{tbl-templates}
\input{../dballe/bufrex/template-aliases.tex}

Alternatively, a template can be specified as {\em category}, {\em subcategory}
and {\em local category}, separated by dots.  For example: ``{\tt 0.255.0}''.
...
{\tt \small idba\_messaggi(h,filename,mode,type,forced\_repcod)} & Work with a message file & \ref{fun-idba_messaggi} \\
