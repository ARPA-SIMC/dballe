...
{\tt idba\_setcontextana(h)}	& Set query parameters to the anagraphical context & \ref{fun-shortcuts} \\
{\tt idba\_enqlevel(h, type, l1, l2)} & Read level data		   & \ref{fun-shortcuts} \\
{\tt idba\_setlevel(h, type, l1, l2)} & Set level data		   & \ref{fun-shortcuts} \\
{\tt idba\_enqtimerange(h, type, p1, p2)} & Read time range data   & \ref{fun-shortcuts} \\
{\tt idba\_settimerange(h, type, p1, p2)} & Set time range data	   & \ref{fun-shortcuts} \\
{\tt idba\_enqdate(h, year, month, day, hour, min, sec)} &
				  Read date information		   & \ref{fun-shortcuts} \\
{\tt idba\_setdate(h, year, month, day, hour, min, sec)} &
				  Set date information		   & \ref{fun-shortcuts} \\
{\tt idba\_setdatemin(h, year, month, day, hour, min, sec)} &
				  Set minimum date for a query	   & \ref{fun-shortcuts} \\
{\tt idba\_setdatemax(h, year, month, day, hour, min, sec)} &
				  Set maximum date for a query	   & \ref{fun-shortcuts} \\
\hline
\end{tabular}
